...
- [`MelProcessor.h`](system/media/audio_utils/include/audio_utils/MelProcessor.h:1) — 247行，头文件定义
...
flowchart TB
    ...
    MEL_PROC -.->|说明| MEL_PROC_NOTE["process()在AudioFlinger<br>写入线程中同步执行<br>不阻塞(只累积能量)"]
    MEL_WORKER -.->|说明| MEL_WORKER_NOTE["回调异步执行<br>在MelWorker独立线程"]
```
