...
>
> 关键事实：
> - 消息包结构为 `aprv2p1_packet_t`（addr+port 二元地址，domain 编码在 addr 高8位）
> - 域ID：`MODEM=3/ADSP=4/APPS=5/SDSP=8/CDSP=9`
> - 对外 API 为 `aprv2_cmd_register/async_send/alloc/free` 等
> - 传输层为 GLink（`apr_glink`），另有 SMDL 备用
> - 真实源码树为 `apr_main.c/aprv2_drv.c/aprv2_rtable.c/aprv2_ipc.c` 等
> - apr_lib 不与 mm-hab/gsl_vm_be/gsl_fe/agm/pal 直接交互（经 `grep -rni "mm-hab|gsl_vm_be|gsl_fe|agm|pal" apr_lib/` = 0 确认）。
...
真实包结构定义在 `core/inc/aprv2p1_packet.h`，名为 `aprv2p1_packet_t`：
...
真实域 ID 定义在 `inc/B2.9/aprv2_ids_domains.h`：
...
真实对外 API 声明在 `core/inc/aprv2_api_i.h`，全部以 `APR_INTERNAL int32_t` 修饰，参数均为对应的 `aprv2_cmd_XXX_t*` 结构体：
...
真实传输层是 **GLink**。`custom/src/aprv2_ipc.c` 中 `#include "apr_glink.h"`，按目标域建立不同的 GLink 链路：
...
apr_lib 使用统一日志宏 `APR_MSG(<优先级>, <格式串>, ...)`，优先级如 `APR_PRIO_ERROR`、`APR_PRIO_HIGH`（见 `apr_glink.c`、`apr_event.c` 等）。日志实现在 `domain/variant/app/src/apr_log.c`。
...
- 真实传输层是 **GLink**（`apr_glink`），备用 SMDL。