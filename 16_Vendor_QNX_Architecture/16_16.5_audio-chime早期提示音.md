...
> **源码说明**：本章依据本机真实源码 `mm-audio-auto/audio-chime/audio_chime_app.c`（单个 **C 文件**，约 948 行）逐函数、逐参数核实。关键事实：
>
> - 纯 **C 全局函数**实现，无任何类；`main()` 串起完整流程
> - dlopen **混用**加载模式：`libacdbloaderclient.so` / `libaudiohalpluginclient.so` 用 **`RTLD_LAZY`**；`libsndmonitor.so` 用 **`RTLD_NOW`**
> - `acdb_loader_send_audio_cal_v6` 为 **8 参数**
> - 播放 **device = 55**；22 是 mixer 路由里的 MultiMedia23 编号
> - 会 dlopen `libaudiohalpluginclient.so` 并调 `audio_hal_plugin_init`
> - SSR 基于 **snd_monitor 回调机制**（`libsndmonitor.so` + `chime_snd_mon_cb`）
> - mixer 路由为 `config_audio_route` + `config_app_type` 两组固定数组写入
...
> 关键辨析：`device = 55` 是 PCM 播放设备号；`MULTIMEDIA_FEDAI_ID = 22`（MultiMedia23）用于 mixer 路由控件名与 ACDB 调用参数。
...
真实播放函数 `play_sample`，PCM 设备号为 **55**：
