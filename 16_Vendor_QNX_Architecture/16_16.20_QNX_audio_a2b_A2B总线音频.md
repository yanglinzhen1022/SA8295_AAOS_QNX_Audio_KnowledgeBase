...
> ### ⚠️ 源码核实（勘误）
>
> 本节旧版包含大量**源码级实现描述**（A2B 核心驱动、节点发现引擎 Discovery Engine、音频流管理器 Stream Manager、故障检测恢复 Fault Handler、AD2429/AD2425/AD2426/AD2428 芯片寄存器时序、主节点初始化时序、`a2b_bus_init`/`a2b_node_discover`/`a2b_stream_open` 等 API、`a2b_types.h`/`a2b_api.h`/`a2b_ad242x.h` 头文件、`liba2b.so`/`liba2b_ad242x.so`/`liba2b_disc.so` 库、`a2b_config.xml` 配置文件），经本地源码核实**全部无依据**：
>
> `audio_a2b` 目录**没有任何 `.c/.h` 源码、没有头文件、没有 XML 配置文件**，只有 `Makefile` 与预编译产物。已将全节重写为"仅预编译库集成"的真实事实版。
>
> | 旧版（虚构/推演） | 真实源码 |
> |---|---|
> | 源码路径 `vendor/qcom/proprietary/audio_a2b/` | `audio_elite/audio_driver/audio_a2b/`（仅 Makefile + prebuilt） |
> | 库 `liba2b.so` / `liba2b_ad242x.so` / `liba2b_disc.so` | `liba2bstack.so` / `liba2bstack-pal.so` / `liba2bstack-protobuf.so` / `liba2bplugin-master.so` / `liba2bplugin-slave.so` |
> | 二进制 `audio-a2b` | `a2b-app` |
> | 头文件 `a2b_types.h` / `a2b_api.h` / `a2b_ad242x.h` | 无头文件（源码不开放） |
> | API `a2b_bus_init` / `a2b_node_discover` / `a2b_stream_open` 等 | 无源码可验证，已删除 |
> | 子模块 Discovery Engine / Stream Manager / Fault Handler | 无源码依据，已删除 |
> | AD242x 寄存器时序、主节点初始化时序 | 无源码依据，已删除 |
> | `a2b_config.xml` / `a2b_persist.xml` 配置文件 | 目录中不存在，已删除 |
>
> **本节可信内容边界**：仅保留①真实预编译库/二进制清单与真实 Makefile 构建信息（来自磁盘）；②A2B 总线的**通用协议背景知识**（来自 Analog Devices 公开资料，明确标注"非本地源码"）。芯片型号、节点数量、时延数字等均为通用协议知识，**并非从本地源码或本地配置读出**。
...
> 具体的从节点芯片型号（如 AD24xx 系列）、每链节点数、时延指标属**硬件与型号相关参数**，随板卡设计不同而变化；本仓库中**没有**任何文件可确认本平台采用的具体型号与参数，故本节不列出确定型号（旧版列出的 AD2429/AD2425/AD2426/AD2428 无本地依据）。
...
> 图中仅表达**已核实**的关系：ADI 预编译产物经本目录 Makefile 拷贝到 BSP staging。A2B 与 ADSP/TDM/auto-audiod 等运行期交互**无本地源码可验证**，故不在此绘制（旧版的 APRv2/ams_lib TDM 注册等交互属推演，已删除）。
...
> 旧版声称的"A2B→TDM→ADSP Hostless 直通""A2B 麦克风→ADAS 安全通道"等属**架构推演**，本仓库无源码或配置可证实，已删除确定性表述。
