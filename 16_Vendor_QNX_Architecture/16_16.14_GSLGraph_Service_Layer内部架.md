...
> ⚠️ **源码核实（勘误）**：GSL 运行于 **QNX PVM**，真实源码位于上述 `audio_reach/gsl/` 路径。原文档标注的 `vendor/qcom/proprietary/args/gsl/inc/` 为推测路径，实际不存在，已全章修正为 QNX 真实路径。已核实 `gsl/src/` 恰为 15 个 `.c`：gsl_main / gsl_graph / gsl_datapath / gsl_subgraph / gsl_subgraph_pool / gsl_shmem_mgr / gsl_msg_builder / gsl_common / gsl_hw_rsc_mgr / gsl_dynamic_module_mgr / gsl_mdf_utils / gsl_global_persist_cal / gsl_rtc / gsl_rtc_main / gsl_spf_ss_state。
...
> ⚠️ **源码核实（勘误）**：源码 `enum gsl_graph_states` 实际只有 **7 个状态**：`GRAPH_IDLE / GRAPH_OPENED / GRAPH_STARTED / GRAPH_STOPPED / GRAPH_CLOSED / GRAPH_ERROR_ALLOW_CLEANUP / GRAPH_ERROR`。**并不存在独立的 PREPARED 状态**——`prepare` 是在 OPENED/STOPPED 态下执行的操作，不改变 `graph_state` 枚举值。下图已按真实枚举绘制。
...
> ⚠️ **源码核实**：三个超时值由条件编译分档——COSIM(WIN32)档为 1800000/1800000/3600000ms；GPR_STUB 单测档为 2000/2000/60000ms；**目标编译默认档(#else)为 1000/1000/60000ms**（车机实际生效值）。上表为默认档真实值，原文档 GRAPH_OPEN=4000、READ_WRITE=1000 有误已修正。
...
> **源码验证**：`audio_reach/ar_osal/`，`api/` 下 14 个抽象头，`src/` 下**只有 `qnx/` 一个实现目录**（14 个 `.c`）。
...
```mermaid
graph TB
    GSL["gsl / gpr / acdb 上层"] -->|"仅调用 ar_osal_* 接口"| API["ar_osal/api<br/>14个抽象头"]
