...
> **源码说明** — 本章依据真实源码（`Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_service_vm/`）编写，要点如下：
>
> 1. **真实源文件**：`src/` 下仅有 `main.c`、`coreinit.c`、`csd_oem_lib_wrapper.c`、`audio_stubs.c`、`ftm_oem_dev_map_wrapper.c`、`gpio_client_stub.c`；`inc/` 仅 `coreinit.h`、`adsp_default_listener.h`。
> 2. **初始化编排**在 `coreinit.c` 的 `audio_service_base_init()`：`DALSYS_InitMod` → `DALSYS_RegisterMod(&gDALModDriverInfoList)` → `CSD_IPC_Server_Start` → `RTPD_IPC_Server_Start` → `init_csd_oem_lib()` → （`#ifdef ENABLE_AUDIO_BACKEND`）`aprv2_vm_init()` / `audio_smmu_vm_be_init()` / `vcodec_plugin_be_start()` / `vapm_be_init()` / `vaudio_gpio_config()`。真实代码中大量 ADSP/AMFS/fastrpc 逻辑被 `#if 0 //tf` 关闭。
> 3. **入口非"事件循环"**：`main()` 用 `getopt(argc, argv, "vhf:U:")` 解析参数，随后 `Resource_Daemonize()` → `audio_service_core_init()`（内部只调 `audio_service_base_init`）→ `sysctrl_init()` → `sysctrl_start(DEVICE_NAME)` 阻塞在 QNX resmgr 框架。
> 4. **设备节点是 `/dev/audio_service`**（resmgr 名，用于单实例互斥检查）。
> 5. **命令行参数**为 `-v`(verbose)/`-h`(usage)/`-f <lpass_config_path>`/`-U <uid:gid>`（SECPOL）。信号仅处理 `SIGTERM`（`handle_sigterm` 直接 `exit(0)`），无 SIGINT/SIGHUP 热重载。
...
> **源码说明**：以下依据 `audio_service_vm/src/main.c` 与 `coreinit.c` 描述真实启动流程。
...
> 说明：守护进程化由无参数的 `Resource_Daemonize()` 完成；配置文件路径通过 `-f` 传入。
...
> 说明：真实源码仅注册 SIGTERM 处理器，进程不支持配置热重载。
...
> **源码说明**：真实核心初始化函数是 `audio_service_base_init(int flags, const char *lpass_config_path)`，无分层步骤、无版本校验、无 ACDB 加载、无 AMS/AGM/GSL 初始化、无 VAPM ioctl 注册、无 goto 清理。以下依据 `audio_service_vm/src/coreinit.c` 描述。
...
> **源码说明**：真实实现（`csd_oem_lib_wrapper.c`）只加载 `libcsd_oem_lib-cdc.so`，用 `RTLD_NOW|RTLD_GLOBAL`，仅解析 **4 个符号**，**无 dlclose 卸载流程**，符号缺失也不回退默认实现。以下依据源码描述。
...
`audio_service_vm` 对外暴露的能力由 `sysctrl_service.c` 注册的这几个 resmgr 回调承载：
...
> **源码说明**：`audio_service_vm` 与 ADSP 的交互是在 [`coreinit.c`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_service_vm/src/coreinit.c:224) 的 `#ifdef ENABLE_AUDIO_BACKEND` 块里，通过一组后端初始化函数拉起 APR/SMMU/vcodec/VAPM/GPIO 后端来完成。
...
- 与 ADSP 的实际消息通道由 `aprv2_vm_init()` 拉起的 **APRv2 VM 后端**承载。
...
> **源码说明**：`audio_service_vm` 目录里与 SSR 相关的符号**全部是 `audio_stubs.c` 中的空桩（`return 0;`）**，且 `coreinit.c` 中真正调用 SSR/监听逻辑的代码块**都被 `#if 0` 关闭**。audio_service_vm 本身**不承担 ADSP SSR 恢复职责**，仅链接这些桩以满足符号依赖。
...
- ADSP SSR 的真实检测与恢复不在 audio_service_vm 主流程内。

### 16.17.9 安全音频与策略仲裁（源码边界说明）

> **源码说明**：`audio_service_vm/` 目录内**不含** safety / secure_audio / parking / adas / cluster 任何字样（grep 全目录为空），**不实现任何独立的安全音频路径**。与"策略/优先级仲裁"真正相关的只有 VAPM 后端。
...
> **源码说明**：按真实源码，audio_service_vm 在整条链路中的角色如下：
...
> - 与 ADSP 的交互为 ENABLE_AUDIO_BACKEND 后端函数（见 16.17.7）
...
> **源码说明**：真实源码里初始化失败的唯一处理就是 **打错误日志 + 直接返回错误码/退出**，并向 **sanity monitor（OEM 工具）** 上报关键错误。无多级服务模式、无 fallback 校准、无 dlopen 失败降级、无“默认 CSD 实现”，无 SSR 恢复步骤、无 poll 超时重试。
...
> - 设备节点是 **`/dev/audio_service`**
> - **无** `kill -HUP` 配置重载、`/proc/asound/apr_status`、`/proc/asound/avmm_sessions`、`acdb_loaded`/`vapm_registered` 状态节点
...
- 用真实日志字样（如 `launching APR backend`）定位启动阶段。

### 16.17.13 配置参数与启动选项

> **源码说明**：`audio_service_vm` **不读取任何 XML 服务配置文件**；它只通过命令行 `getopt` 解析少量启动选项，可选 `-f` 传入 LPASS 配置路径。
