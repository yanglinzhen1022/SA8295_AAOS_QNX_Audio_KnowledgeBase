...
3. **电源监控（委托）** — 电源相关处理委托给`AutoPower`（`mAutoPower`，`VHAL_EXT`下）订阅VHAL电源属性
...
> **说明**：真实实现按声卡(`snd_card`)遍历`MAX_SESSION`个会话，使用表驱动的`g_audio_route[][]`（mixer控件名/值）与`g_audio_pcm[][]`（PCM设备号）配置。
