...
# 应包含 auto-audiod 持有的 wakelock 名 "audio_daemon"
# 若wakelock丢失，系统可能进入suspend导致hostless断开
```

> **⚠️ 源码核实（勘误）**：早期文档称 wakelock 名为 `hostless_merc`/`hostless_a2b`，经核对真实源码
> [`auto_audio_ext.cpp`](vendor/qcom/proprietary/mm-audio-auto/auto-audiod/auto_audio_ext.cpp:67) 中
> `static const char* wakelock_name = "audio_daemon";`，auto-audiod 通过 `wakelock_acquire()`
> 向 `/sys/power/wake_lock` 写入的是**统一名称 `audio_daemon`**（并非按 hostless 通路分名）。
> `wake_lock_fd` 在 `wakelock_init()` 中以 `O_WRONLY|O_APPEND` 打开 `/sys/power/wake_lock`，
> 由 `wakelock_level` 引用计数控制 acquire/release（level==1 时才实际写入）。
...
```bash
cat /sys/power/wake_lock | grep audio_daemon   # auto-audiod 的 wakelock 名
tinymix -D 0 | grep "Port Mixer"
...
| 阶段 | open() | start() |
|------|--------|---------|
| GSL API | `gsl_open()`(内部`gsl_graph_open()`) | `gsl_ioctl(GSL_CMD_START)`(内部`gsl_graph_start()`) |
| 功能 | 创建并配置Graph | 启动数据处理 |
| 资源分配 | DSP分配资源、构建模块链 | 开始PCM数据读写 |
| 参数配置 | 媒体格式、设备接口、校准数据 | 无(配置在open阶段完成) |
| 类比 | "搭建舞台、安装设备" | "演出开始" |

> **⚠️ 源码核实（勘误）**：早期文档将 DSP 操作写作 `gsl_open_graph()`/`gsl_start()`，经核对真实源码
> [`gsl_intf.h`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_ar/audio_driver/audio_reach/gsl/api/gsl_intf.h:656)：
> 对外公共 API 为 **`gsl_open()`**（非 `gsl_open_graph`），启动无独立 `gsl_start()` API，而是通过
> `gsl_ioctl()` 下发命令 **`GSL_CMD_START`**（=0x0，gsl_intf.h:86）触发；`GSL_CMD_PREPARE`(=0x1)
> 为可选的预初始化命令。GSL 内部对应函数为 [`gsl_graph_open()`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_ar/audio_driver/audio_reach/gsl/inc/gsl_graph.h:269) 与 [`gsl_graph_start()`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_ar/audio_driver/audio_reach/gsl/inc/gsl_graph.h:418)。
...
# 常见 GSL_VM_OPCODE（gsl_vm_msg.h 真实枚举值）：
# 4  = GSL_VM_OPCODE_OPEN    (gsl_open)
# 5  = GSL_VM_OPCODE_CLOSE   (gsl_close)
# 13 = GSL_VM_OPCODE_IOCTL   (gsl_ioctl，start/stop 经此下发 GSL_CMD_START/STOP)
# 14 = GSL_VM_OPCODE_READ    15 = GSL_VM_OPCODE_WRITE
# 无响应 = MM-HAB通道断开
```

> **⚠️ 源码核实（勘误）**：早期文档列出 `0x0001=GSL_CMD_OPEN_GRAPH`/`0x0002=CLOSE_GRAPH`/`0x0003=START_GRAPH`
> 属虚构命名与取值。真实枚举见 [`gsl_vm_msg.h`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_ar/audio_driver/gsl_be/inc/gsl_vm_msg.h:52)：
> `GSL_VM_OPCODE_OPEN = 4`、`GSL_VM_OPCODE_CLOSE = 5`、`GSL_VM_OPCODE_IOCTL = 13`。GSL VM 层没有独立的
> "START_GRAPH" opcode——图启动是通过 `GSL_VM_OPCODE_IOCTL(=13)` 携带 `GSL_CMD_START`(=0x0) 命令完成的（详见 16.14）。
...
# 常见domain(APRv2真实值): ADSP=4, MODEM=3, APPS=5
```

> **⚠️ 源码核实（勘误）**：早期文档写 "domain: ADSP=1, MODEM=3"，其中 ADSP=1 有误。经核对
> [`aprv2_ids_domains.h`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver/inc/B2.9/aprv2_ids_domains.h:44)：
> `APRV2_IDS_DOMAIN_ID_ADSP_V = 4`、`MODEM_V = 3`、`APPS_V = 5`、`SIM_V = 1`（仿真域）。
> 注意 APRv2 域 ID 与 GPR（AudioReach 新协议）域 ID 体系不同——GPR 侧 ADSP=2/APPS=3（详见 16.14）。
...
**A**: audio_driver_vm是QNX音频栈最底层驱动，真实子模块包含avmm_lib(AVMM)、vapm_lib(VAPM)、apr_vm_lib(APRv2 VM)、audio_smmu_vm_lib(SMMU VM BE)、apr_fe_lib(APR前端)、vcodec_lib(虚拟编解码)、amfs_lib等。
...
**Step 4 — 检查编解码/前端接口**：
```bash
# vcodec_lib提供虚拟编解码，apr_fe_lib为APR前端
slog2info | grep -E "vcodec|apr_fe|amfs"
# 通信失败 = 与ADSP交互异常
```

> **⚠️ 源码核实（勘误）**：早期文档将 audio_driver_vm 五模块列为 AVMM/VAPM/APRv2 VM/**VDSP**/SMMU VM BE，其中
> "VDSP虚拟DSP接口" 无对应源码目录，疑为虚构。经核对真实目录
> [`audio_driver_vm/`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver_vm)，实际子模块为：
> `avmm_lib`(AVMM)、`vapm_lib`(VAPM)、`apr_vm_lib`(APRv2 VM)、`audio_smmu_vm_lib`(SMMU VM BE)、
> `apr_fe_lib`(APR前端)、`vcodec_lib`/`v_codec_plugin_fe`(虚拟编解码)、`amfs_lib`。请以此为准。
...
graph TB
    subgraph audio_driver_vm[audio_driver_vm驱动层]
        AVMM[avmm_lib<br/>音频虚拟化管理器]
        VAPM[vapm_lib<br/>虚拟音频策略管理器]
        APRV2[apr_vm_lib<br/>APRv2虚拟化接口]
        APRFE[apr_fe_lib<br/>APR前端]
        VCODEC[vcodec_lib<br/>虚拟编解码]
        SMMU[audio_smmu_vm_lib<br/>SMMU虚拟化后端]
    end
...
cat /sys/power/wake_lock | grep audio_daemon
# auto-audiod 统一持有名为 "audio_daemon" 的 wakelock；wakelock丢失 = 系统可suspend → Hostless断开
```
