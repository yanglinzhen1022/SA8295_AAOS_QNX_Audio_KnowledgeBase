...
graph TB
    subgraph ACDB_Storage[ACDB存储]
        ACDB_QNX[QNX侧ACDB文件 - /sys/platform/acdb/<br/>实际生效源 adp_8295_ar 或 adp_8295]
...
- **QNX 侧主导**：QNX 持有并装载 ACDB 文件(`/sys/platform/acdb/`,按平台为 `adp_8295_ar` 或 `adp_8295`),由 QNX AM 在启动时加载并**直连推入 ADSP**,是实际生效源
...
> **路径澄清**：ACDB 双域各有一份同源文件,但**实际生效的是 QNX(PVM) 侧 `/sys/platform/acdb/`**;Android(GVM) 侧 `/vendor/etc/acdbdata/`(注意目��名是 `acdbdata` 而非 `acdb`)文件存在但直连路径被架空。真正把校准写入 ADSP 的 GSL 本体在 QNX 侧,GVM 只有代理 `libar-gsl_fe.so`。详见 [16.7 ACDB校准数据(Android与QNX双域共享)](16_16.7_ACDB校准数据Android与QNX双域共享.md) 与 [16.14 GSL 内部架构](16_16.14_GSLGraph_Service_Layer内部架.md)。
...
| ACDB文件(QNX侧) | /sys/platform/acdb/ | **实际生效源**,QNX AM 装载直连 ADSP(adp_8295_ar / adp_8295) |
| ACDB文件(Android侧) | /vendor/etc/acdbdata/ | 文件存在但直连路径被架空/占位,��用经 gsl_fe→HAB 转 QNX 执行 |
