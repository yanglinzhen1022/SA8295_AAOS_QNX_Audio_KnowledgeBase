...
> **源码说明**：`audio_driver_vm` 真实位于 QNX 侧
...
> **源码说明**：SMMU 映射真实函数为 `audio_smmu_vm_be_map()`/`audio_smmu_vm_be_map_v2()`（见 [`audio_smmu_vm_be.c`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver_vm/audio_smmu_vm_lib/src/audio_smmu_vm_be.c:95)）；`habmm_socket_send()` 为真实 MM-HAB API，在 SMMU VM 后端等各 backend 内直接调用，并非由 AVMM 统一转发。
...
> **源码说明**：依据 [`avmm_i.h`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver_vm/avmm_lib/inc/avmm_i.h) 与
...
**规模上限**：`#define AVMM_MAX_SIZE (32)`（`avmm_i.h`）——该常量为后端名字符串长度上限，源码中不存在"最多 N 个 GVM 会话"这一限制。
> **源码说明**：AVMM 不是基于 opcode 分发的消息处理器，而是 client(backend)+connection 的连接生命周期管理器（`avmm_notify_connect`/`avmm_notify_disconnect`）；控制命令转发实际由各 backend（如 `aprv2_vm_be.c`）自行处理。
...
> **源码说明**：`vapm_cb_param` 真实字段仅 `handle`/`effect`/`enable` 三项（**无 `duck_level`**）；`vapm_effect_t` 真实成员含 `VAPM_EFFECT_NONE`/`MAX`；回调返回值为 `int32_t`。VAPM 后端经 MM-HAB 以 S2C 命令（`VAPM_CMDID_S2C_DUCK/MUTE/SOFF`）下发效果。上文 DUCK/MUTE/SOFF 时序图为逻辑示意。
...
> **源码说明**：APRv2 VM 真实实现见 [`aprv2_vm_i.h`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver_vm/apr_vm_lib/inc/aprv2_vm_i.h) 与 [`aprv2_vm_be.c`](Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver_vm/apr_vm_lib/src/aprv2_vm_be.c)，采用命令协议 + per-VM 会话模型，如下。
...
> **源码依据**：`apr_vm_lib/src/aprv2_vm_be.c`、`apr_vm_lib/inc/aprv2_vm_i.h`
>
> **源码说明**：APRv2 VM 后端为每个 GVM 维护一个会话对象，不用路由表，也不做地址翻译，仅按 session 转发 APR 包。
...
> **源码说明** — 权威源码：`apr_vm_lib/src/aprv2_vm_be.c`
>
> - APR 域值：MODEM=3 / ADSP=4 / APPS=5（见 `aprv2_ids_domains.h`）
...
> **源码依据**：`audio_smmu_vm_lib/src/audio_smmu_vm_be.c`
>
> **源码说明**：真实实现名为 `audio_smmu_vm_be_map()` / `audio_smmu_vm_be_map_v2()`，映射请求经 MM-HAB（`habmm_socket_send`）跨 VM 传递，底层 SMMU/IOMMU 由 QNX 侧承接，后端仅登记映射项。
...
> **源码说明**：源码中不存在 fault 回调；后端不实现 IOMMU fault handler，也不向 AVMM 发送 fault 通知。跨 VM 传输统一通过 `habmm_socket_send(session->hab_handle, ...)`。
...
> **源码依据**：`apr_vm_lib/src/vdsp.c`、`apr_vm_lib/inc/vdsp_internal.h`
>
> **源码说明**：真实的 VDSP 是一个 **client + apr_call 抽象**，不是多 DSP 实例管理，句柄类型均为 opaque。
...
> **源码依据**：`audio_ar/audio_driver/gsl_be/inc/gsl_vm_be.h` / `gsl_vm_msg.h` / `src/gsl_vm_be.c`
>
...
> **源码说明**：AVMM/APRv2 VM 用 client/backend + 命令 ID 协议，不用 opcode 分发；而 gsl_vm_be 的 `GSL_VM_OPCODE_*` 是另一套真实存在的协议，二者不可混淆。

**GVM 连接管理（AVMM 真实模型）**：

> **源码依据**：`audio_driver_vm/inc/avmm.h`
>
> **源码说明**：`AVMM_MAX_SIZE=32` 是 `be_name[AVMM_MAX_SIZE]` **后端名字符串长度上限**，不是会话/连接数量上限。AVMM 以 **client → connection 链表**动态管理。
...
> **源码依据**：`audio_driver_vm/inc/avmm.h`
>
> **源码说明**：AVMM 只负责 GVM 监控与 client/connection 生命周期，**不做数据收发**（数据面由各后端自持 hab_handle 通过 `habmm_socket_send/recv` 直接处理）。以下为 `avmm.h` 真实公共 API。
...
> **源码依据**：`audio_driver_vm/inc/vapm.h`
>
> **源码说明**：VAPM 是**权限申请 + S2C 通知模型**，不是"施加 effect / 设置 duck 电平"的直接接口。回调 `vapm_callback_t` 返回 `int32_t`，回调参数 `vapm_cb_param` 仅含 `{handle, effect, enable}`，**无 `duck_level`**。
...
> **源码依据**：`apr_vm_lib/src/aprv2_vm_be.c`
>
> **源码说明**：真实模型以 AVMM client + per-GVM `aprv2_vm_be_session_t`（含 `dispatch_pkt_q`）承载，命令用 `APRV2_VM_CMDID_REGISTER/DEREGISTER/ASYNC_SEND` + 事件 `APRV2_VM_EVT_RX_PKT_AVAILABLE`。以下为真实公共 API。
...
> **源码依据**：`apr_vm_lib/src/vdsp.c` / `inc/vdsp_internal.h`
>
> **源码说明**：真实 VDSP 是 **client + apr_call 抽象**（opaque `vdsp_t` / `vdsp_client_t`），非多 DSP 实例管理。
...
> **源码依据**：`audio_smmu_vm_lib/src/audio_smmu_vm_be.c`
>
> **源码说明**：真实后端只有 init/deinit/map/map_v2，映射请求经 `session->hab_handle` 用 `habmm_socket_send` 下发，映射项挂在 `session->smmu_map_q`。
...
> **源码说明**：源码普遍返回通用 `int32_t` 负值或 `APR_E*` / `AR_E*` 系列错误码，无专用错误码宏体系。AVMM 不设会话数量上限（`AVMM_MAX_SIZE=32` 是后端名字符串长度，见 16.16.9）；APRv2 VM 无地址路由表（见 16.16.11.3）；SMMU 无独立 `fault_handler`（见 16.16.7）。实际排障以 slog2 日志与源码返回值为准。
...
> **源码说明**：`gsl_vm_be`（位于 `audio_ar/gsl_be/`）与 `audio_driver_vm`（AVMM/APRv2/VAPM/SMMU）是**并列的两套跨 VM 后端**，不是上下游串行链。GSL 数据面走 gsl_vm_be 的 opcode 通道；APR 控制面走 APRv2 VM；权限仲裁走 VAPM。三者经各自的 MM-HAB 通道并行接入 QNX 侧，最终汇聚到 ADSP。
...
> **源码说明**：VAPM 是**权限申请 + S2C 通知模型**（见 16.16.5 / 16.16.11.2），通过回调把 `effect/enable` 通知 GVM，由 GVM 侧自行响应。
