...
>
> **澄清二：QNX 侧配置/校准文件路径为真实路径。** QNX 侧 ACDB 真实位于 `boards/audio_driver/adp_8295/acdb_oem_datafiles/audcal/acdbdata/`（分类 `.acdb` 文件：Speaker_cal / Handset_cal / Codec_cal / General_cal / Hdmi_cal 等）；GSL 子图/路由配置为 `gsl/api/gsl_subgraph_driver_props.xml`。旧版表格中的 `/sys/platform/audio/`、`mixer_paths.xml`、`adsp_avs_config.acdb.pvm` 等为推测路径，已在 16.12.6 更正。
