...
Primary HAL(AudioReach版)是8295平台上AudioReach架构的Audio HAL 7.0实现，位于PAL层之上，将Android框架的音频语义转换为PAL API调用。AR版专为AudioReach架构设计，通过PAL→AGM→gsl_fe→MM-HAB→gsl_vm_be→GSL→APM路径与DSP交互。

**源码路径**：`vendor/qcom/opensource/audio-hal-ar/primary-hal/hal-pal/`

> **源码核实（2026 核对，与本机真实源码逐项比对一致）**：
> - 目录与文件真实存在：`audio-hal-ar/primary-hal/hal-pal/`（`AudioDevice.{h,cpp}` / `AudioStream.{h,cpp}` / `AudioVoice.{h,cpp}`）；Legacy 版为 `audio-hal/primary-hal/hal/`，二者以路径区分。
> - VSID：`VOICEMMODE1_VSID=0x11C05000`、`VOICEMMODE2_VSID=0x11DC5000`、`MAX_VOICE_SESSIONS=2`（`AudioVoice.h`）✓
> - 延迟常量：`LOW_LATENCY=13ms / DEEP_BUFFER=70ms / PCM_OFFLOAD=30ms / MMAP=3ms / ULL=4ms`（`AudioStream.h`，源码单位为 `*1000LL` 微秒）✓
> - VoIP 缓冲：`320/640/1280/1920`、音量 `MIN=-6000 / MAX=0 mB`（`AudioDevice.h`）✓
> - UseCase 枚举与字符串标识、车载流常量（`CAR_AUDIO_STREAM_MEDIA=0 / NAV_GUIDANCE=2 / REAR_SEAT=16 / MAX=32`）、核心方法（`CreateStreamOut/CreateAudioPatch/GetPalDeviceIds/GetPalStreamType/CreateMmapBuffer`）均与源码一致 ✓
...
| CAR_AUDIO_STREAM_NAV_GUIDANCE | 2 | 导航流 |
...
| CALL_INACTIVE | BASE_CALL_STATE | 通话未激活（相对基值） |
| CALL_ACTIVE | BASE_CALL_STATE+1 | 通话激活（相对基值） |
