...
> **源码说明（本机源码核实）**：
> 1. GVM SA8295 的资源配置是**单一文件** `resourcemanager_gvmauto8295_adp_star.xml`。根元素为 **`<resource_manager_info>`**，且 `bt_codecs`/`controls`/`device_profile`/`config_voice` 都是这一个文件内部的子节点，并非独立文件。
> 2. 用例→图 KV 映射真实位于 `usecaseKvManager.xml`。
...
> **源码说明**：真实控制名为 `PLUGIN_CONTROL_VOLUME` / `PLUGIN_CONTROL_VOLUME_BOOST` / `PLUGIN_CONTROL_HD_VOICE` / `PLUGIN_CONTROL_AUDIO_BUFFER` / `PLUGIN_CONTROL_AUDIO_LATENCY`，通过 `default` 指向 `.so` 插件并用 `plugin_usecase` 限定作用范围。
...
> **源码说明**：`modepair` 的 `key` 是调制模式标识，`value` 是对应 VSID(Voice Session ID)。
...
> **源码说明**：
> - **无 `acdb_id` 字段**——设备到 ACDB 的关联由图元数据（KV）与 ACDB 数据本身承载。
> - 扬声器 `back_end_name` 真实为 **`TDM-LPAIF-RX-PRIMARY`**。
...
> A2B 是车机多喇叭分区的关键。相关播放用例在 UCM(见 16.8) 中以 `A2B`/`A2B2` 用例呈现，与此处设备对应。
