...
> **真实磁盘路径（对照源码）**：AGM 属于 Android(GVM) 侧用户态开源组件，源码位于 `vendor/qcom/opensource/agm/`。该目录为 AOSP vendor 树下的逻辑路径，本知识库所在工作区未同步 vendor 源码，实际以设备端 vendor 分支为准。AGM 通过 gsl_fe 代理经 MM-HAB 跨 VM 调用 QNX(PVM) 侧 GSL，最终由 ADSP APM 构建图。

```
vendor/qcom/opensource/agm/
├── api/
│   └── agm_api.h              # AGM 对外 API(会话/设备/图 生命周期)
├── inc/
│   ├── device.h               # Device 对象(后端设备抽象)
│   ├── graph.h                # Graph 对象(GSL 图句柄封装)
│   └── session_obj.h          # Session 对象(流会话状态机)
├── src/
│   ├── agm.c                  # AGM 核心实现(API 分发)
│   ├── device.c               # 设备管理(枚举/开关/参数)
│   ├── graph.c                # 图管理(open/prepare/start/stop)
│   └── session_obj.c          # 会话对象管理
└── service/                   # AGM service(IPC 服务端)
```

> **交互对照**：AGM 命令体系对接 ADSP 侧 `apm_api.h`(SPF 框架 APM 命令)，图操作最终经 GSL → GPR → ADSP APM 完成。详见 [16.10.4 AGM与APM(SPF)交互](16_16.10_AGMAudio_Graph_Manager深度解.md) 与 [16.14 GSL/Graph Service Layer内部架构](16_16.14_GSLGraph_Service_Layer内部架.md)。
