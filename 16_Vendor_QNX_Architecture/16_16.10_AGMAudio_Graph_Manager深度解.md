...
> **重大澄清（旧版臆造 → 真实源码）**：
> - **源码路径**：旧版写 `vendor/qcom/opensource/agm/` 下有顶层 `api/ inc/ src/` 目录 —— **错误**。本机真实源码顶层为 `ipc/ plugins/ service/ snd_parser/`，AGM 服务核心全部在 `service/` 下：公共 API 在 `service/inc/public/agm/agm_api.h`，私有头在 `service/inc/private/agm/`（device.h/graph.h/session_obj.h/metadata.h/graph_module.h/agm_priv.h），实现在 `service/src/`（agm.c/graph.c/session_obj.c/device.c/metadata.c/graph_module.c/device_hw_ep.c）。
> - 旧版把 `agm_key_vector_gsl`/`agm_meta_data_gsl` 归入公共 API `agm_api.h` —— **错误**。二者是 AGM **私有元数据**结构，定义/使用于 `service/inc/private/agm/metadata.h` 及各私有对象头，公共 `agm_api.h` 对外暴露的是 `agm_key_value`/`agm_tag_config`/`agm_cal_config`(柔性数组 `kv[]`) 等。
...
> **重大澄清**：旧版 16.10.2 只罗列了数据结构、完全遗漏了上述公共 API 函数集。真实 `agm_api.h` 的核心价值正是这批 `agm_session_*`/`agm_aif_*` 函数，PAL 即通过它们驱动 AGM。
...
> **澄清**：旧版把 `agm_key_vector_gsl { num_kvs; kvs* }` / `agm_meta_data_gsl { gkv; ckv; sg_props }` 写成公共 API 结构 —— 错误。公共层用的是 `agm_tag_config`/`agm_cal_config`(柔性数组 `kv[]`)；`agm_key_vector_gsl`/`agm_meta_data_gsl` 属 **私有** metadata.h(见 16.10.3)。
...
struct agm_media_config {
    ...
    uint32_t data_format;          // 数据格式(旧版遗漏)
};
...
> **澄清**：旧版枚举名写成 `agm_format`、首值从 `PCM_S8=0` 起、并遗漏 `INVALID/WMASTD/WMAPRO/VORBIS/AMR_WB_PLUS/QCELP/MAX` —— 均与真实 `enum agm_media_format` 不符，已按源码修正。
...
struct agm_buffer_config {
    ...
    size_t   max_metadata_size; // 每缓冲最大元数据(旧版遗漏)
};
```
...
> **澄清**：旧版臆造了 `enum hw_ep_intf {CODEC_DMA...AUDIOSS_DMA}`、`enum pcm_intf_idx {PRIMARY...SENARY}`、`hw_ep_cdc_dma_i2s_tdm_config.sd_line_idx` 等 —— 真实接口类型是 **宏**(CODEC_DMA/MI2S/TDM/AUXPCM/SLIMBUS)，且 device_obj 含 **虚拟设备/group_data/refcount** 等旧版遗漏的车载关键字段。

#### Graph对象(graph.h)

Graph 对象在源码中是 `struct graph_obj`(旧版误写 `graph` 且臆造 `enum graph_state`；graph.h 中并无该枚举)。真实 API：
...
// 动态图操作(AudioReach 实时拓扑变更关键，旧版遗漏)
...
> **澄清**：旧版臆造了 `GRAPH_CLOSED..GRAPH_STOPPED` 状态机并遗漏 `graph_add/graph_change/graph_remove` 这三个 AudioReach **动态图变更**核心 API。
...
> **澄清**：旧版把 `aif_pool` 写成 `struct aif_pool*`(真实是 `struct listnode`)，把回声参考/环回写成独立 `agm_ec_ref`/`agm_loopback` 结构(真实是 `ec_ref_aif_id+ec_ref_state`/`loopback_sess_id+loopback_state` 各两个平铺字段)。
