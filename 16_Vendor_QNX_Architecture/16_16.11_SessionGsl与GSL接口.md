...
> **本章关键说明（对照真实源码，2026 核对）**：以下按真实源码逐条说明。核对依据：
...
>
> **澄清三：GSL 真实 API。** 真实是 `gsl_open` / `gsl_close` / `gsl_ioctl`（用 `GSL_CMD_START`/`GSL_CMD_STOP` 命令控制启停）。
>
> **澄清四：`gsl_key_vector` 字段名是 `num_kvps`。**
>
> **澄清五：PayloadBuilder 真实方法名是 `payloadXxxConfig` + `populateXxxKV`。**
...
> **澄清**：真实 Session 子类中 **不存在** `buildGkv()` / `buildCkv()` / `buildTkv()` / `configureGraph()` / `sendCalData()` 这些成员方法。KV 的构建由 `PayloadBuilder::populateStreamKV` 等完成；图配置经 ALSA mixer control 下发给 AGM。
...
typedef void *gsl_handle_t;  // 不透明句柄
...
> **澄清**：真实 `gsl_intf.h` 中 **不存在** `gsl_open_graph(&gkv, &handle)`、`gsl_start(handle)`、`gsl_stop(handle)` 这类分立函数。真实 open 是 `gsl_open(gkv, ckv, &handle)`（一次传两个 KV 向量），启停用 `gsl_ioctl(handle, GSL_CMD_START/STOP, ...)`。
...
> **澄清**：真实 `PayloadBuilder.h` 中 **不存在** `buildGkv()` / `buildCkv()` / `buildTkv()` / `buildPayload()` / `buildPcmTdmPayload()` / `pcm_tdm_module_config` 等方法与结构。真实是上述 `populateXxxKV` + `payloadXxxConfig`。
...
> **澄清**：`kvh2xml.h` 是 **自动生成的键值宏头文件**（真实位置 `mm-audio-headers/acdbdata/kvh2xml.h`），只是 `#define` 一堆 KEY_/VALUE_ 宏与 tag 枚举，供 ACDB/PayloadBuilder 引用。
...
4. **PayloadBuilder 真实方法 `populateXxxKV` + `payloadXxxConfig`**，返回/输出普通 C++ 容器。
