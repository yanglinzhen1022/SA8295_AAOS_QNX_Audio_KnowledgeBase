...
> **与旧文档差异**：旧文档把`serviceDied`写成`AutoPower`自身覆写(`hidl_death_recipient`)、带`mLock`/`mSubscribed`清理。真实源码里`serviceDied`属于内嵌`VehicleHalDeathRecipient`，签名是`(uint64_t, const wp<hidl::base::V1_0::IBase>&)`，只转调`handleVehicleHalDeath()`。
...
> **与旧文档差异**：真实`initVehicle`没有`mLock`/读取初始状态/`mSubscribed=true`/`subscribePowerProperties()`这些步骤；订阅通过`reqVehicleProperties`数组一次性`subscribe`。旧文档的`subscribePowerProperties()`、`SubscribeFlags::EVENTS_FROM_ANDROID`等均为推演。

### 16.3.6 onPropertySet()电源状态处理（真实生效路径）

> **关键澄清**：真实源码里`onPropertyEvent`**是空实现**（仅打印属性数量），电源事件处理**全部在`onPropertySet`**里完成，且只处理`AP_POWER_STATE_REPORT`下的两个状态。旧文档把处理逻辑放在`onPropertyEvent`并列举了9种`case`——均与真实源码不符。
...
> **与旧文档差异**：旧文档在`DEEP_SLEEP_ENTRY`/`SHUTDOWN_START`/`SHUTDOWN_CANCELLED`等多个状态里也调用了hostless，且调用的是`AutoPower::enable_hostless_all()`成员方法——真实源码**只在`DEEP_SLEEP_EXIT`和`SHUTDOWN_POSTPONE`两处**、调用的是**全局函数**`auto_audio_ext_*`。
...
> **重要澄清（与旧文档差异）**：`enable_hostless_all` / `disable_hostless_all` / `enable_hostless` / `disable_hostless` **不是 `AutoPower` 的成员方法**，而是 `auto_audio_ext.cpp` 中定义的**C全局函数**（`auto_audio_ext.h` 用 `__BEGIN_DECLS` 声明）。`AutoPower::onPropertySet` 只是**调用者**。旧文档把它们写成 `AutoPower::enable_hostless(session_type)` 的 switch/mixer 实现是虚构的。真实实现是**表驱动**，详见 [16.2.8]，此处仅列签名与调用关系。
...
> **与旧文档差异**：旧表列出"系统关机SHUTDOWN_PREPARE→disable_hostless_all"和"关机取消SHUTDOWN_CANCELLED→enable_hostless_all"两条——真实 `AutoPower::onPropertySet` **没有这两个 case**，hostless 只在 `REPORT_DEEP_SLEEP_EXIT` 和 `REPORT_SHUTDOWN_POSTPONE` 生效。
...
> **与旧文档差异（重要）**：旧文档给出的 `AutoPower::disable_hostless_all()` 内 `hasActiveSafetyAudio()` / `scheduleDeferredDisable()` 延迟禁用逻辑是**虚构的**。真实 `AutoPower::onPropertySet` 在 `REPORT_SHUTDOWN_POSTPONE` 分支**直接、无条件**调用全局函数 `auto_audio_ext_disable_hostless_all()`，没有安全音频检查、没有延迟禁用、AutoPower 也没有 `disable_hostless_all` 成员方法。本节的"安全音频优先"策略表属于**框架层设计意图/上层策略**（可能由 CarAudioService / CPMS 侧的关机延迟机制承载），并非 auto-audiod 的 AutoPower 实现。
