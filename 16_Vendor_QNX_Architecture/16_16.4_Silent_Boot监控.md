...
sequenceDiagram
    ...
    participant ANDR as "Android域 init"
    ...
    HW->>ANDR: Android启动
    ANDR->>ANDR: init读取boot reason
    ANDR->>ANDR: 设置persist.audio.silent_boot=1

    Note over HW,AP: 阶段2 早期组件抑制

    ANDR->>CHIME: 启动audio-chime
    ...
    ANDR->>ANDR: persist.audio.silent_boot=0
    CHIME->>CHIME: 检测属性变化
