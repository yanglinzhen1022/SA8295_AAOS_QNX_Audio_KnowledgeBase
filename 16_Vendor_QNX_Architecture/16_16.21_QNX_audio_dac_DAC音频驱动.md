...
> ### 源码说明
>
> 真实芯片是 **PCM1681**（`enum pcm1681_reg_map`，代码注释 "pcm1681 control"）；ioctl 仅 `case CMD_SET_DAC_MUTE (0x1001) / CMD_SET_DAC_FMT (0x1002)` 两个命令；核心结构为 `audio_dac_main_t` / `audio_dac_context_t` / `audio_dac_init_config_t` / `mute_payload_t` / `fmt_payload_t`；初始化参数由 `audio_dac_init_config_t`（i2c_device/i2c_addr）传入，目录中无 XML 配置；默认增益由内部 `audio_dac_set_default_gain`（写默认增益 224）设置，无对外音量 cmd；单例 `static audio_dac_main`，无 SSR 逻辑；普通库 + `audio_dac_ioctl` 分发，无 resmgr/dispatch。
>
...
> `audio_dac_ioctl` 中 `switch(cmd)` 只有这两个 `case`（0x1001/0x1002）。
