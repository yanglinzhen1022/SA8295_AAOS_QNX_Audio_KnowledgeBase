...
> **重大澄清（本机源码核实）**：
> 1. Legacy 目录是**扁平结构**，`*_avs_config.acdb.pvm` 与各 `*_cal.acdb` 同级，无 `avs_config/`、`ivi_config/`、`adas_config/` 子目录。
...
> **重大澄清（本机源码核实）**：以上参数**均封装在二进制 `.acdb` 数据库内部**。真实 `.acdb` 无法以文本查看，只能经 **QACT 工具**打开 `workspaceFile.qwsp` 工程编辑。设备与 ACDB ID 对应以 §16.6.6 `acdb-id-mapper.h` 真实映射为准（如 `SPEAKER_RX=15`、`BT_SCO_RX=22`）。
...
> **重大澄清（本机源码核实）**：以上 `*_avs_config.acdb` / `*.acdb.pvm` 均为**二进制文件**（`file` 判定为 `data`），不含可读文本字段，其配置由校准工具生成、以二进制形式存储。
...
> **重大澄清（本机源码核实）**：真实车载配置仅为上述两份 **.acdb 路径清单**，用于指示加载器在 IVI/ADAS 场景下装载哪些二进制校准数据库、从哪个挂载点（`//ifs//` vs `//mnt//`）读取。
