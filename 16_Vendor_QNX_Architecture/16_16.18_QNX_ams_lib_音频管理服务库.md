...
> **⚠️ 源码核实（勘误）**：旧版本文档称 ams_lib「经MM-HAB→APR发送命令」「管理安全音频图(倒车雷达/ADAS)」「与VAPM策略交互保证图优先级」。经核实本地源码 `Qnx/apps/qnx_ap/AMSS/multimedia/audio/audio_elite/audio_driver/ams_lib/`（inc/ams.h、src/ams_impl.c/ams_platform.c/ams_util.c）：
> - **下发机制并非 APR**：真实路径是 `csd_open(CSD_OPEN_AMS_CONTROL)` 获取句柄 + `csd_ioctl(h, CSD_AMS_CMD_TUNNEL/ENABLE_ENDPOINT/DISABLE_ENDPOINT/SET_CLK_ATTR, ...)`（见 `src/qnx_driver/platform_drv.c`）。aprv2_msg_if.h 仅提供消息格式定义，ams_lib 不直接调用独立的 apr_lib。
...
> ⚠️ **源码核实（勘误）**：旧版将 `ams_graph_handle_t` 描述为 `uint32_t` 并定义 `AMS_GRAPH_HANDLE_SAFE_OFFSET 0x80000000`（安全图句柄偏移）——**源码中不存在**。真实为 `void*`，无安全图/Android 图句柄区间划分，无 `AMS_GRAPH_HANDLE_INVALID` 宏。
...
> ⚠️ **源码核实（勘误）**：真实枚举**从 `TDM1 = 1` 起**（非旧版 `TDM1 = 0`），且**只有 TDM1~TDM12**，源码中**不存在** `MI2S_RX/MI2S_TX/PDM_TX`。TDM8~TDM12 在源码注释中标记为 High Speed Interface 0~4。
...
> ⚠️ **源码核实（勘误）**：真实事件枚举只有 `AMS_EVENT_SSR_STARTED/COMPLETED` 两个，回调原型为 `void (*)(ams_event_id_t, void*)`。旧版列出的 `GRAPH_OPENED/CLOSED/ERROR/HW_INTF_READY` 等事件、以及 `ams_init_params_t`（含 `driver_fd`/`apr_handle`/`hw_intf_table`）**均不存在**——`ams_init` 真实签名为 `ams_status_t ams_init(ams_session_t *amss)`，无任何入参结构体。
...
> ⚠️ **源码核实（勘误）**：旧版 `ams_graph_open/close/start/stop/set_param/get_param`、`ams_hw_interface_open/close/connect/disconnect`、`ams_register_event_cb/ams_register_hw_intf` 等 API **在源码中均不存在**；`apr_graph_open_cmd`/`graph_ctx_t`/`APR_DOMAIN_ADSP`/`afe_port_open_cmd`/`apr_send_sync` 代码路径**全部虚构**。ams_lib 不经 APR/AFE Port，而是经 CSD ioctl 隧道（见 16.18.12）。
...
`ams_set_param` 以 `(module_id, param_id, param_size, data)` 四元组定位并下发模块参数，最终封装为 CSD ioctl 隧道命令送达 ADSP 上对应 CAPIv2 模块。无旧版列出的 `AMS_PARAM_VOLUME/ROUTING/CALIBRATION` 等固定枚举——参数 ID 由具体 CAPIv2 模块定义。


### 16.18.5 图管理完整生命周期

> ⚠️ 源码核实（勘误）：旧版本节使用的 `ams_graph_open` / `ams_hw_interface_connect` / `ams_hw_interface_disconnect` / `ams_graph_start` / `ams_graph_close` 等 API 名称、以及 `SUSPENDED` / `RECOVERING` / `CONFIGURED` 图状态、`apr_lib` / `APR_GRAPH_OPEN` / `AFE_PORT_OPEN` 等交互链路**均不存在于源码**。真实实现见下。
>
> 权威依据：`ams_lib/src/ams_impl.c`（对外 API `ams_open_graph`/`ams_start_graph`/`ams_stop_graph`/`ams_close_graph`/`ams_destroy_graph`）与 `ams_lib/src/ams_platform.c`（底层 `ams_platform_open_graph`/`start`/`stop`/`close`/`apply_prop`）。
...
> ⚠️ 源码核实（勘误）：旧版本节的 `AMS_HW_IF_TDM*` 枚举名（真实为 `AMS_HW_INTERFACE_TDM*`）、`AFE Port ID`（0x9001 等）、各 TDM 的“典型用途/方向/安全音频”绑定表、以及安全图句柄 `0x8xxxxxxx` / Android 图句柄 `0x0xxxxxxx` **均为推演，源码中不存在**。真实 TDM 定义见 `ams_lib/inc/ams.h` 的 `ams_hw_interface_id_t` 枚举与 `ams_endpoint_t.tdm_params` 结构。
...
> ⚠️ 源码核实（勘误）：旧版本节的 `AMS_PARAM_CALIBRATION/VOLUME/ROUTING/MEDIA_FORMAT` 固定参数枚举、`ams_volume_param_t`/`ams_routing_param_t`/`ams_media_format_param_t` 结构体、APM/AFE/ASM/ADM/LSD_CAL 校准类型表、以及 `apr_lib`/`APR_GRAPH_SET_PARAM` 链路**均为推演，源码中不存在**。真实参数通道见 `ams_lib/src/ams_impl.c` 的 `ams_set_param`/`ams_get_param`。
...
> **⚠️ 源码核实（勘误）**
> 旧版本节称"ams_lib 是 AGM（Audio Graph Manager）Service 的底层接口库，负责将 AGM 请求翻译为 ADSP 可执行的 **APR 命令**"，并给出 `agm_session_open→ams_graph_open`、`agm_session_connect→ams_hw_interface_connect`、`APR Graph/AFE 命令` 层等映射表。
> 经核实 `ams_lib` 全部源码（`inc/`、`src/`）：**不存在任何 `agm`/`apr` 字样**（`grep -rni "agm\|apr" = 0`）。ams_lib 既不是 AGM 的底层库，也不产生 APR/AFE 命令。它是一套**独立的图管理库**，通过 **CSD ioctl 隧道**（opcode `DSP_AMS_CMD_*`）直接与 ADSP 通信。旧版的 AGM/APR 分层、`agm_session_*` 映射表、`ams_graph_open`/`ams_hw_interface_connect` 等 API **均为虚构**，予以删除。
...
> **⚠️ 源码核实（勘误）**：旧版本节虚构了 `graph_ctx_t{ handle; params; state; connected_hw_intfs 位图; ref_count; }` 结构与全局 `graph_list`/`graph_list_lock`。真实源码中：
> - 图上下文类型是 `struct ams_graph`（非 `graph_ctx_t`），且**无 `ref_count` 引用计数、无 `connected_hw_intfs` 硬件接口位图**；
> - 图链表**归属于每个 `ams_session`**（`session->gr_lst`），**不是全局单例**；一个进程可持有多个 session，各自独立管图；
> - 并发保护用 `pthread_rwlock_t`（读写锁），链表级一把、每个图节点各一把（`struct ams_graph` 内含 `nxt_el`/`prev_el`/`rwlock`），非旧版的单一 `PTHREAD_MUTEX_INITIALIZER`。
...
> **⚠️ 源码核实（勘误 · 整节重写）**
> 旧版本节名为"安全音频图管理"，声称：安全图由 QNX 侧独立创建、`ams_graph_open(SAFE_FLAG)` 设置 **handle 高 bit**、安全图句柄 `0x8001`、经 `AFE_PORT_OPEN(TDM10)`、`VAPM 优先级仲裁器`保证 QNX 图优先、Android 图 handle 高 bit=0 等。
> 经核实 ams_lib 全部源码：**不存在 `safe`/`secure`/`0x8000`/`vapm` 任何字样**（`grep -rni = 0`）。ams_lib **没有"安全音频图"这一概念**，图句柄 `ams_graph_handle_t` 是 `void*`（无高 bit 语义），也没有 VAPM 仲裁器、没有 `AFE_PORT_OPEN`。旧整节均为虚构，予以删除。
>
> ams_lib 中真正与"独立/隔离通路"相关的、**源码可证的机制**是**端点共享标志 `flags`**，本节据此重写。
...
要点：
- "隔离/独占"的真实实现是 `EXCLUSIVE`——由 ams_lib 全权配置该 TDM 端点；这与旧版虚构的"handle 高 bit + VAPM 仲裁"完全不同。
...
> **⚠️ 源码核实（勘误 · 整节重写）**
> 旧版本节称：SSR 事件为 `AMS_EVENT_SSR_DOWN`/`AMS_EVENT_SSR_UP`，经 `apr_lib` 通知；ams_lib 遍历 `graph_list` 标记图为 `SUSPENDED`；恢复阶段 ams_lib **自动重建所有图**（`APR_GRAPH_OPEN`/`AFE_PORT_OPEN`/`APR_GRAPH_CONNECT`/`APR_GRAPH_SET_PARAM`/`APR_GRAPH_START`），并按 **P0-P4 优先级**（倒车雷达/ADAS/eCall/通话/导航/媒体）分 5 批渐进恢复、安全图优先、失败重试 3 次、ACDB 重新推送。
> 经核实 `ams_impl.c` 的 `ams_ssr_callback_function`（41-166 行）：以上**全部为虚构**。真实机制没有 apr_lib、没有 SSR_DOWN/UP 事件、没有 SUSPENDED 状态、没有 P0-P4 优先级、没有安全图优先、没有 ams_lib 自动重建图、没有 ACDB 重推。本节据真实源码重写。
...
> **重建责任在调用方，不在 ams_lib**：SSR 完成后 ams_lib 只把图重置到 `INIT` 并发 `AMS_EVENT_SSR_COMPLETED`，**不会自动 open/start 任何图**。上层收到该事件后，需自行按需重新 `ams_open_graph` → `ams_start_graph` 恢复。这与旧版"ams_lib 按优先级自动重建 5 批图"的描述根本不同。

### 16.18.11 上层调用方与完整处理链路

> ⚠️ **源码核实（勘误）**：旧版描述了一条 `Android App → PAL Stream → gsl_fe → MM-HAB → gsl_vm_be → ams_lib → apr_lib → ADSP` 的跨 VM 链路，并称 ams_lib 通过 `apr_lib` 发 APRv2 命令、经 `agm_session_open/ams_graph_open` 映射。**这些在 ams_lib 源码中完全不存在**：`grep -rni "gsl|pal|mm-hab|hab|agm|apr"` 全为 0。ams_lib 是一个**普通用户态库**，由**同进程的调用方直接链接并调用其 `ams_*` API**，内部经 **CSD ioctl** 直达 ADSP，不涉及 PAL/GSL/AGM/APR/MM-HAB 任何一层。本节据 `ams_lib/`、`ams_test/` 源码勘误。
...
> 说明：`grep -rni "ams_create_graph|ams_open_graph"` 在 audio 目录下命中的调用方仅 `ams_test/`。ams_lib 面向的是**同进程直接 API 调用**模型，而非旧版描述的跨进程/跨 VM RPC 模型。
...
> ⚠️ **源码核实（勘误）**：旧版称 ams_lib「通过 apr_lib 发 APR 协议消息」，并列出 `open(/dev/snd/audio_driver_vm)`、`write()/read()/ioctl(APR_REGISTER_CB)`、`RPMSG/GLINK` 等接口。**源码中不存在**：`ams_lib` 无 apr、无设备节点 open、无 write/read。真实交互通过 **CSD（`csd.h`）** 完成，见 `ams_lib/src/qnx_driver/platform_drv.c`。
...
> ⚠️ **源码核实（勘误）**：旧版称 ams_lib 使用 QNX `slog2`，且列出 `AMS_LOGE/W/I/D/V` **五级**日志与一批不存在的日志字符串（`SSR_DOWN/UP`、`graph_recovery`、`安全图 handle(0x8xxx)` 等）。**源码不同**：日志宏在 `src/ams_log.h`，实际为 **`AMS_LIB_LOGE/AMS_LIB_LOGI/AMS_LIB_LOGD` 三级**，底层调用 `logger_log(QCLOG_AMSS_MM_APPS_CAD, ...)`；无 `AMS_LOGW/AMS_LOGV`，无 slog2 直接调用。本节据 `ams_log.h` 勘误。
...
> ⚠️ **源码核实（勘误）**：旧版源码树（`ams_core.c/ams_graph.c/ams_hw_interface.c/ams_param.c/ams_apr_handler.c/ams_ssr.c/ams_debug.c`、`config/hw_intf_table.xml`、`README.md`）为虚构，磁盘上均不存在。真实顶层为 `inc/`、`src/`、`aarch64/`、`common.mk`、`Makefile`。
