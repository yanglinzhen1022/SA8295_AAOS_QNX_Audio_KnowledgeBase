...
> | 旧版（虚构/推演） | 真实源码 |
> |---|---|
> | 芯片 **PCAL6524 / TCA9539**、多扩展器实例、24bit/8bit | 源码不指定具体型号，是通用 I2C GPIO 扩展器（Port0/Port1 寄存器），单例 `audio_expander_main` |
> | API `expander_open/close/reset/gpio_set/gpio_get/get_status` | 真实为 `audio_expander_init/open/ioctl/close/deinit` 五个 |
> | 结构 `aud_handle_t{expander_id,num_pins,chip_type}` / `expander_pin_config_t` / `mux_config_t` | 真实为 `audio_expander_main_t` / `audio_expander_context_t` / `audio_expander_init_config_t` + payload 结构 |
> | 头文件 `aud_expander_db.h` / `aud_expander_cfg.h` | **不存在** |
> | `expander_config.xml` 配置文件 | **不存在**——参数由 `audio_expander_init_config_t`（i2c_device/i2c_addr）传入 |
> | "引脚数据库 / XML 解析 / 通用 GPIO 读写" | 无源码依据，真实是固定 3 个命令控制 DAC 使能/源切换 |
>
