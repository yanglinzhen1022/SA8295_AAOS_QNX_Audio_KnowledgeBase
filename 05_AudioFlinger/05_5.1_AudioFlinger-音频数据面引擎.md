...
AudioFlinger是Android音频系统的**数据面核心引擎**，运行在`mediaserver`进程中，是所有音频数据流的最终汇聚点和分发中心。其核心职责涵盖：
...
```mermaid
sequenceDiagram
    participant MS as main_mediaserver
...
```mermaid
graph TB
    MS[main_mediaserver<br/>主线程] --> AF_THREAD[AudioFlinger Binder线程池<br/>处理IAudioFlinger Binder调用]
