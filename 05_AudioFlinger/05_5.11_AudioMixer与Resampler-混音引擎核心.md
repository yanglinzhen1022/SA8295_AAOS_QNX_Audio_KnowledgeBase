...
flowchart TD
    ...
    M -->|否| O{需要RESAMPLE?}
    O -->|是| P{单声道MONO?}
    P -->|是| Q[TRACKTYPE_RESAMPLEMONO\nMIXTYPE_STEREOEXPAND]
    P -->|否| R{立体声+useStereoVolume?}
    R -->|是| S[TRACKTYPE_RESAMPLESTEREO\nMIXTYPE_MULTI_STEREOVOL]
    R -->|否| T[TRACKTYPE_RESAMPLE\nMIXTYPE_MULTI]
    O -->|否| U{单声道MONO?}
    U -->|是| V[TRACKTYPE_NORESAMPLEMONO\nMIXTYPE_MONOEXPAND]
    U -->|否| W{立体声+useStereoVolume?}
    W -->|是| X[TRACKTYPE_NORESAMPLESTEREO\nMIXTYPE_MULTI_STEREOVOL]
    W -->|否| Y[TRACKTYPE_NORESAMPLE\nMIXTYPE_MULTI]
```
