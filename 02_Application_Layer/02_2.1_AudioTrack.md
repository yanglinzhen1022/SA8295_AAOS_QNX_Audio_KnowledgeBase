...
stateDiagram-v2
    state "初始化状态 mState" as InitStates {
        STATE_UNINITIALIZED : mState=0\n创建前或release后
        STATE_INITIALIZED : mState=1\n构造成功
        STATE_NO_STATIC_DATA : mState=2\nMODE_STATIC未写入数据
        --
        STATE_UNINITIALIZED --> STATE_INITIALIZED : 构造成功\nnative_setup返回OK
        STATE_INITIALIZED --> STATE_NO_STATIC_DATA : MODE_STATIC\n首次write前
        STATE_NO_STATIC_DATA --> STATE_INITIALIZED : write()返回>0
    ...
    state "播放状态 mPlayState (Offload模式)" as OffloadStates {
        STOPPING : mPlayState=4\n等待STREAM_END回调
        PAUSED_STOPPING : mPlayState=5\n从STOPPING暂停
        --
        PLAYING --> STOPPING : stop()\n(Offload)
        STOPPING --> STOPPED : NATIVE_EVENT_STREAM_END回调
        STOPPING --> PAUSED_STOPPING : pause()\n(Offload, 在STOPPING期间)
        PAUSED_STOPPING --> STOPPING : play()\n(恢复到STOPPING)
        PAUSED_STOPPING --> STOPPED : NATIVE_EVENT_STREAM_END\n(即使暂停仍可能到达)
    }
```
