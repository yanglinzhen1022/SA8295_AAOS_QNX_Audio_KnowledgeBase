...
[`EngineBase`](frameworks/av/services/audiopolicy/engine/common/include/EngineBase.h:28) 是Android音频策略引擎的**基类骨架**，实现了 [`EngineInterface`](frameworks/av/services/audiopolicy/engine/interface/EngineInterface.h:46) 定义的策略查询接口，同时将**路由决策核心逻辑**留给子类通过纯虚函数 `getDevicesForProductStrategy()` 实现。这种设计实现了**策略引擎的可插拔替换**——OEM可以选择默认引擎(`enginedefault`)或可配置引擎(`engineconfigurable`)，甚至实现完全自定义的路由算法。
