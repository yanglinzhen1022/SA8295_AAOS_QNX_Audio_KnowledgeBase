...
| ResourceManager | `vendor/qcom/opensource/pal/resource_manager/` | 资源管理器，核心调度中心（`inc/` + `src/`） |
| PayloadBuilder | `vendor/qcom/opensource/pal/session/{inc,src}/PayloadBuilder.{h,cpp}` | GSL/APM 载荷构建器（**位于 session 目录下，无独立 `PayloadBuilder/` 目录**） |
...
| Sound Trigger HAL | `vendor/qcom/opensource/audio-hal-ar/st-hal/` | 语音触发 HAL 实现（AudioReach C++ 版：SoundTriggerDevice/SoundTriggerSession，经 PAL） |

> ⚠️ **源码核实（勘误）**：Sound Trigger HAL 路径经本地源码核实应为 `audio-hal-ar/st-hal/`（AudioReach C++ 版），而非早前误写的 `audio-hal/ar/enhanced-v2/`。详见 [15.17](15_15.17_QC_Sound_Trigger_HAL.md)。

## 15.1.7 PAL 核心源码目录树

> ⚠️ **源码核实（重大勘误）**：以下目录树已按本地真实源码 `vendor/qcom/opensource/pal/` 重写。真实结构为 **`inc/` + `src/` 分离**（头文件与实现分目录），而非早前所画的 `.cpp/h` 混放。真实根目录只有 `Android.mk`（无 `Android.bp`），入口头文件为 `PalApi.h`（无 `pal_api.h`），无 `PalApi.cpp` / `Pal.cpp` 仅 `Pal.cpp` 存在于根，`ContextManager` 为独立 `context_manager/{inc,src}` 目录，`SndCardMonitor` 位于 `resource_manager/src/` 下，无独立 `PayloadBuilder/` 目录。

```
vendor/qcom/opensource/pal/
├── Android.mk                       # 编译脚本（无 Android.bp）
├── configure.ac / Makefile.am       # autotools 构建（QNX/宿主侧）
├── ar-pal.pc.in                     # pkg-config 模板
├── Pal.cpp                          # PAL 核心逻辑
├── PalApi.h                         # PAL 公共 API 头文件
├── PalCommon.h                      # 公共定义与工具宏
├── PalDefs.h                        # 数据类型定义（pal_buffer/pal_stream_type_t 等）
├── PalAudioRoute.h                  # 音频路由定义
├── stream/                          # Stream 层（inc/ + src/）
│   └── src/
│       ├── Stream.cpp               # Stream 基类
│       ├── StreamCommon.cpp         # Stream 公共实现
│       ├── StreamPCM.cpp            # PCM 流
│       ├── StreamCompress.cpp       # Compress Offload 流
│       ├── StreamSoundTrigger.cpp   # 语音唤醒流
│       ├── StreamACD.cpp            # ACD（声学上下文检测）流
│       ├── StreamInCall.cpp         # 通话流
│       ├── StreamNonTunnel.cpp      # NON_TUNNEL 流
│       ├── StreamContextProxy.cpp   # 上下文代理流
│       ├── StreamSensorPCMData.cpp  # 传感器 PCM 数据流
│       └── StreamUltraSound.cpp     # 超声流
├── device/                          # Device 层（inc/ + src/）
│   └── src/
│       ├── Device.cpp               # Device 基类
│       ├── Speaker.cpp              # 扬声器
│       ├── SpeakerProtection.cpp    # 扬声器保护
│       ├── SpeakerMic.cpp           # 扬声器路径麦克风
│       ├── Headphone.cpp            # 有线耳机
│       ├── HeadsetMic.cpp / HeadsetVaMic.cpp
│       ├── Handset.cpp / HandsetMic.cpp / HandsetVaMic.cpp
│       ├── Bluetooth.cpp            # 蓝牙（A2DP/HFP/LEA）
│       ├── USBAudio.cpp             # USB 音频（非 USB.cpp）
│       ├── DisplayPort.cpp          # DisplayPort 音频
│       ├── A2BSpeaker.cpp / A2B2Speaker.cpp  # A2B 扬声器
│       ├── FMDevice.cpp / ExtEC.cpp / RTProxy.cpp
│       ├── HapticsDev.cpp / UltrasoundDevice.cpp
├── session/                         # Session 层 + PayloadBuilder（inc/ + src/）
│   └── src/
│       ├── Session.cpp              # Session 基类
│       ├── SessionAlsaPcm.cpp       # ALSA PCM 会话
│       ├── SessionAlsaCompress.cpp  # ALSA Compress 会话
│       ├── SessionAlsaVoice.cpp     # ALSA Voice 会话
│       ├── SessionAlsaUtils.cpp     # ALSA 会话工具
│       ├── SessionAgm.cpp           # AGM 会话（NON_TUNNEL）
│       ├── PayloadBuilder.cpp       # 载荷构建器（位于此，无独立目录）
│       ├── SoundTriggerEngine.cpp / SoundTriggerEngineCapi.cpp / SoundTriggerEngineGsl.cpp
│       ├── ACDEngine.cpp / ContextDetectionEngine.cpp
│       └── (SessionGsl 仅存 inc/SessionGsl.h，无 .cpp，不参与运行)
├── resource_manager/                # ResourceManager（inc/ + src/）
│   └── src/
│       ├── ResourceManager.cpp      # 资源管理器主体
│       └── SndCardMonitor.cpp       # 声卡状态监控（位于此）
├── context_manager/                 # 上下文管理器（独立目录，inc/ + src/）
├── ipc/HwBinders/                   # HwBinder IPC 实现
├── configs/                         # 平台配置
│   ├── bengal/  kona/  lahaina/  monaco/  msmnile/  taro/
│   └── (monaco 对应 SA8295 平台)
├── plugins/                         # 插件体系（编解码器插件）
├── utils/                           # 工具集（inc/ + src/）
│   └── src/
│       ├── PalRingBuffer.cpp        # 环形缓冲区
│       ├── ChargerListener.cpp      # 充电状态监听
│       ├── SoundTriggerPlatformInfo.cpp / SoundTriggerXmlParser.cpp / SoundTriggerUtils.cpp
│       └── ACDPlatformInfo.cpp      # ACD 平台信息
└── test/                            # 测试代码
```
...
PAL 的编译构建基于 Android 构建系统。

> ⚠️ **源码核实**：真实 PAL 根目录仅含 `Android.mk`（未见 `Android.bp`），同时提供 `configure.ac` / `Makefile.am`（autotools，用于 QNX/宿主侧构建）。以下产物名为架构层面示意，具体产物以实际编译配置为准。
