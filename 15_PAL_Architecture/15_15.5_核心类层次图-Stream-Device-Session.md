...
> [← 上一个](15_15.4_流类型_pal_stream_type_t.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_15.6_架构地位.md)]
...
| 空间音频 | `SPATIAL_AUDIO` | OUTPUT | SessionAlsaPcm | 空间音频渲染 |
...
graph TB
    ...
    subgraph AudioSession[音频数据通路Session]
        SessionAlsaPcm[SessionAlsaPcm<br/>AGM/GSL音频图管理]
...
- 音频数据通路 Session（SessionAlsaPcm / SessionAlsaPcm / SessionAlsaCompress / SessionAlsaVoice）面向音频数据传输
...
**SessionAlsaPcm 与 SessionAlsaPcm 的选择逻辑**：

在 AudioReach 架构下，Session::makeSession() 的选择取决于平台配置和流类型：
- 需要 DSP 处理（音效、回声消除等）→ SessionAlsaPcm
