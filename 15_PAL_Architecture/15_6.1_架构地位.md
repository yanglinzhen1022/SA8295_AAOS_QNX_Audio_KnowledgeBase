...
> [← 上一个](15_5.1_核心类层次图-Stream-Device-Session.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_7.1_核心职责.md)

---

## 15.6 ResourceManager架构地位总览
...
| 成员 | 类型 | 说明 |
|------|------|------|
| `active_streams_ll` | `list<Stream*>` | 低延迟活跃流列表 |
| `active_streams_db` | `list<Stream*>` | 深缓冲活跃流列表 |
| `active_streams_ull` | `list<Stream*>` | 超低延迟活跃流列表 |
| `active_streams_comp` | `list<Stream*>` | 压缩流活跃列表 |
| `active_streams_st` | `list<Stream*>` | 音频触发流活跃列表 |
| `active_streams_proxy` | `list<Stream*>` | 代理流活跃列表 |
| `active_streams_incall_rec` | `list<Stream*>` | 通话录音活跃列表 |
| `active_streams_incall_music` | `list<Stream*>` | 通话音乐活跃列表 |
| `active_streams_voice` | `list<Stream*>` | 语音通话活跃列表 |
| `mActiveStreams` | `list<Stream*>` | 全部活跃流汇总列表 |
| `stream_instances[]` | `int[PAL_STREAM_MAX]` | 各类型流实例计数 |

> 流按类型分列表存储，便于并发流策略快速遍历同类型流。`mActiveStreams`是所有流的汇总视图，`stream_instances[]`用于快速判断某类型流是否存在。
...
| `audio_hw_mixer` | `struct audio_hw_mixer*` | ALSA硬件混音器句柄 |
...
| 成员 | 类型 | 说明 |
|------|------|------|
| `concurrentStreamInfo` | `concurrent_stream_config` | 并发流配置（优先级/互斥规则） |
| `devicePcmIdMap` | `map` | 设备到PCM ID的映射 |
| `nonHubStreamFeIds` | `vector<int>` | 非Hub模式流前端ID |
| `gainDbToLevelMap` | `map` | 增益dB到Level映射 |

### 通话与BT类

| 成员 | 类型 | 说明 |
|------|------|------|
| `voiceVSID` | `相关结构` | 通话VSID配置（CS/IMS/VoLTE等） |
| `btCodecFormat` | `相关结构` | BT编解码器格式配置 |
| `currentBTCodec` | `相关结构` | 当前BT编解码器实例 |

### 同步与状态类

| 成员 | 类型 | 说明 |
|------|------|------|
| `mResourceManagerMutex` | `pthread_mutex_t` | 核心互斥锁 |
| `ssrHandled` | `bool` | SSR处理状态标志 |
| `isAdspUp` | `bool` | ADSP在线状态 |
| `isCbUp` | `bool` | CDSP在线状态 |
...
| 中高 | `PAL_STREAM_LL_PLAYBACK` | 低延迟播放（如按键音） |
...
- **`getStreamAttrPriority(pal_stream_type_t type)`** — 获取流类型对应的优先级值
- **并发规则配置** — 在`concurrent_stream_config` XML节点中定义，支持自定义并发/互斥规则
...
sequenceDiagram
    ...
    RM->>RM: 设置ssrHandled=true
    RM->>RM: 标记isAdspUp=false
...
> **ADSP vs CDSP**：ADSP（Audio DSP）处理音频计算，CDSP（Compute DSP）处理语音触发检测。两者独立SSR，ResourceManager分别跟踪`isAdspUp`和`isCbUp`状态。
...
| XML节点 | 解析目标 | 存储位置 |
|---------|----------|----------|
| `<device_profile>` | 设备能力描述 | `deviceInfo` |
| `<pcm_ids>` | PCM前端ID列表 | `listAllPcmPlaybackFrontEnds` / `listAllPcmRecordFrontEnds` |
| `<compress_ids>` | Compress前端ID列表 | `listAllCompressPlaybackFrontEnds` |
| `<backend_ids>` | 后端ID与名称映射 | `listAllBackEndIds` |
| `<ec_ref>` | EC参考设备映射 | `deviceInfo`内嵌 |
| `<concurrent_stream>` | 并发流规则 | `concurrentStreamInfo` |
| `<voice_vsid>` | 通话VSID配置 | `voiceVSID` |
| `<bt_codec>` | BT编解码器配置 | `btCodecFormat` |
| `<gain_db_to_level>` | 增益映射表 | `gainDbToLevelMap` |
| `<custom_config>` | 自定义模块配置 | 按模块存储 |

### 解析流程

1. **打开XML文件** — 使用libxml2解析器
2. **根节点遍历** — 按节点名分发到对应解析函数
3. **设备配置解析** — 创建Device对象，填充设备能力（采样率、位深、通道等）
4. **ID池构建** — 解析前端/后端ID，构建可用ID池
5. **映射关系建立** — EC ref映射、设备到PCM ID映射等
6. **策略配置加载** — 并发流规则、增益映射等
...
| VSID类型 | 说明 | 典型场景 |
|----------|------|----------|
| CS_VSID | 电路交换语音 | 传统2G/3G通话 |
| IMS_VSID | IMS语音 | VoLTE/VoWiFi |
| VOIP_VSID | VoIP语音 | 第三方VoIP应用 |

通话流打开时，ResourceManager根据`pal_stream_type_t`和通话模式选择对应VSID，配置到SessionAlsaVoice中。
