...
AGM 是 PAL 的直接下层，接口定义在 `agm/service/inc/public/agm/agm_api.h` 中：
...
**PAL 层**：`pal_stream_write()` 接收 `struct pal_buffer`（定义于 `PalDefs.h`）：

```c
// PAL层数据封装（真实定义，struct pal_buffer）
struct pal_buffer {
    uint8_t *buffer;          // 数据指针
    size_t size;              // 数据字节数
    size_t offset;            // 有效字节起始偏移
    struct timespec *ts;      // 时间戳
    uint32_t flags;           // 标志位
    size_t metadata_size;     // metadata 缓冲区大小
    uint8_t *metadata;        // metadata 缓冲区（可含多个 metadata）
    pal_extern_alloc_buff_info_t alloc_info; // 外部分配缓冲区信息
};
```

> ⚠️ **源码核实**：真实结构体名为 `struct pal_buffer`（非 `pal_stream_buffer` / `pal_stream_buffer_t`），字段包含 `ts`（`struct timespec*`）与 `metadata`/`alloc_info`，而非早前所写的四字段简化版。
...
```c
// AGM层共享内存写入（真实签名，agm_api.h）
int agm_session_write(uint64_t hndl, void *buff, size_t *count);
// hndl 为 agm_session_open 返回的会话句柄
// 数据被写入与ADSP共享的ring buffer，ADSP感知新数据到达
// 带元数据的变体：agm_session_write_with_metadata(uint64_t hndl, struct agm_buff *buff, ...)
```
