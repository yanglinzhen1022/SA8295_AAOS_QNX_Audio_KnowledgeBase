...
struct pal_stream_attributes {
    ...
    struct pal_stream_info info;             // 流附加信息(union)
    ...
    char *bus_addr;                          // 总线地址(car场景)
    uint32_t hal_flags;                      // HAL标志
    audio_channel_mask_t ch_mask;            // 声道掩码
    audio_format_t format;                   // 音频格式
};
```

> ⚠️ **源码核实（勘误）**：旧版将 `in_media_config`/`out_media_config` 标为 `uint32_t`，并遗漏 `info`/`bus_addr`/`hal_flags`/`ch_mask`/`format` 字段。真实 `PalDefs.h` 中二者均为 `struct pal_media_config`，完整字段如上（详见15.4.3）。
...
struct pal_buffer {
    uint8_t *buffer;                         // 数据指针
    size_t size;                             // 数据大小（字节）
    size_t offset;                           // 有效数据偏移量
    struct timespec *ts;                     // 时间戳(指针)
    uint32_t flags;                          // 标志
    size_t metadata_size;                    // 元数据大小
    uint8_t *metadata;                       // 元数据缓冲
    pal_extern_alloc_buff_info_t alloc_info; // 外部分配缓冲信息
};
```

> ⚠️ **源码核实（勘误）**：旧版将 `buffer` 标为 `void*`、`ts` 标为 `struct pal_time_stamp` 值类型。真实 `PalDefs.h` 中 `buffer` 为 `uint8_t*`、`ts` 为 `struct timespec*` 指针，并含 `flags`/`metadata`/`metadata_size`/`alloc_info`（详见15.4.9）。
...
struct pal_volume_data {
    ...
    struct pal_channel_vol_kv volume_pair[];  // 通道-音量键值对数组
};
...
> ⚠️ **源码核实（勘误）**：真实 `PalDefs.h`(788-796) 中数组名为 `volume_pair`（非`vol_pair`），键值对字段为 `vol`（非`volume`）。
...
| `PAL_PARAM_ID_KW_TRANSFER_LATENCY` | 关键词传输延迟 | SoundTrigger关键词传输 |

> ⚠️ **源码核实（勘误）**：旧版列出的 `PAL_PARAM_ID_SESSION_ALSA_MMAP`/`PAL_PARAM_ID_LATENCY_MODE` 在 `PalDefs.h` 中不存在，已删除。`PAL_PARAM_ID_UIEFFECT` 真实存在。
...
| `PAL_PARAM_ID_DEVICE_CAPABILITY` | 设备能力查询 | 查询支持的采样率/编码格式 |

> ⚠️ **源码核实（勘误）**：以上 param_id 均在 `PalDefs.h` 中真实存在。旧版列出的 `PAL_PARAM_ID_SSR` 不存在，已删除；`DEVICE_CAPABILITY` 描述原"季节点"为笔误，应为"采样率"。
...
typedef int32_t (*pal_stream_callback)(
    pal_stream_handle_t *stream_handle, // 触发事件的流句柄
    uint32_t event_id,           // 事件ID
    uint32_t *event_data,        // 事件数据(uint32_t指针)
    uint32_t event_data_size,    // 事件数据大小
    uint64_t cookie              // 用户cookie
);
```

> ⚠️ **源码核实（勘误）**：旧版遗漏了首参 `pal_stream_handle_t *stream_handle`，并将 `event_data` 标为 `void*`。真实 `PalDefs.h` 中签名首参为流句柄、`event_data` 为 `uint32_t*`（详见15.4.9）。

### 流回调事件类型

| 事件ID | 说明 | 触发场景 |
|--------|------|----------|
| `PAL_STREAM_CBK_EVENT_WRITE_READY` | 非阻塞写就绪 | 非阻塞write完成时 |
| `PAL_STREAM_CBK_EVENT_DRAIN_READY` | Drain完成 | pal_stream_drain()数据排空 |
| `PAL_STREAM_CBK_EVENT_PARTIAL_DRAIN_READY` | 部分Drain完成 | partial drain完成时 |
| `PAL_STREAM_CBK_EVENT_READ_DONE` | 异步读取完成 | 非阻塞read完成时 |
| `PAL_STREAM_CBK_EVENT_ERROR` | 流错误 | DSP错误、SSR等 |

> ⚠️ **源码核实（勘误）**：旧版虚构了 `WRITE_DONE`/`ST_READY`/`ST_CH Recognition`/`ACD_DETECTED` 等事件。真实 `PalDefs.h`(613-617) 中 `pal_stream_callback` 事件仅5个，如上表；且真实使用 `WRITE_READY`（非`WRITE_DONE`），并含 `PARTIAL_DRAIN_READY`。
