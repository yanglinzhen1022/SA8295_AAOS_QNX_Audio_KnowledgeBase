...
> [← 上一个](15_9.1_配置文件列表.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_11.1_编解码器插件_pluginscodecs.md)

---

## 15.8 HAL-PAL适配层架构深度解析

HAL-PAL适配层是Audio HAL 7.0与PAL API之间的桥梁，将Android框架的音频语义（audio_hw_device、audio_stream_out/in接口）转换为PAL的Stream/Device/Session抽象。源码位于`vendor/qcom/opensource/audio-hal-ar/primary-hal/hal-pal/`，由AudioDevice、AudioStreamOut、AudioStreamIn、AudioVoice四大核心类和audio_extn扩展模块构成。
...
### 15.10.8.7 audio_extn扩展模块详解

HAL-PAL适配层通过`audio_extn/`目录下的扩展模块支持各种增强功能。这些模块通过函数指针或直接调用方式与核心类交互。

| 扩展模块 | 源码路径 | 说明 | HAL-PAL交互方式 |
|----------|----------|------|-----------------|
| auto_hal | audio_extn/auto_hal.c | 车载音频区域管理 | Car Stream→PAL Car Stream映射 |
| battery_listener | audio_extn/battery_listener.c | 电池状态监听 | 影响PA供电和功耗策略 |
| FM | audio_extn/fm.c | FM收音机 | FM Tuner→PAL Stream |
| Hfp | audio_extn/hfp.c | HFP蓝牙通话 | SCO→PAL Voice Session |
| soundtrigger | audio_extn/soundtrigger.c | 语音触发 | PAL SVA/Hotword Stream |
| Gef | audio_extn/gef.c | Generic Effect Framework | Offload Effect Plugin |
| Gain | audio_extn/gain.c | 增益控制 | PAL Volume Control |
| a2dp | audio_extn/a2dp.c | A2DP蓝牙音频 | PAL A2DP Stream+Offload |
| custom_compress | audio_extn/custom_compress.c | 自定义压缩格式 | PAL Compress Offload |
| spatial_audio | audio_extn/spatial_audio.c | 空间音频 | Head Tracking+PAL |
| display_port | audio_extn/display_port.c | DisplayPort音频 | DP→PAL HDMI设备 |
| keep_alive | audio_extn/keep_alive.c | 低功耗保活 | 最小化DSP活动 |
| incall_music | audio_extn/incall_music.c | 通话中音乐 | 通话+音乐并发 |
| compressor | audio_extn/compressor.c | 压缩器 | PAL Compress Session |

#### A2DP Offload扩展

A2DP Offload是audio_extn中最复杂的扩展模块之一，将蓝牙音频编解码从Host侧Offload到DSP执行：

1. **启动流程**：`audio_extn_a2dp_start()`→获取BT编解码器配置→创建PAL A2DP Stream→`pal_stream_start()`
2. **Offload编解码**：DSP端执行SBC/AAC/aptX/LDAC编解码，Host侧仅传输压缩数据
3. **设备变更**：A2DP连接/断开时触发`audio_extn_a2dp_set_device()`→路由切换
4. **延迟监控**：通过`audio_extn_a2dp_is_ready()`检查A2DP链路状态
