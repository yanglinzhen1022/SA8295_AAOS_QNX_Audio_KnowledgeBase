...
> [← 上一个](15_15.17_QC_Sound_Trigger_HAL.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](../16_Vendor_QNX_Architecture/README.md)
...
|------------|--------------------------|---------|
| `StreamPCM` | LOW_LATENCY, DEEP_BUFFER, ULTRA_LOW_LATENCY, PROXY, HAPTICS, RAW, PLAYBACK_BUS, VOIP, VOIP_RX, VOIP_TX, GENERIC, PCM_OFFLOAD | PCM播放/录制 |

> **⚠️ 源码核实（重大勘误）**：经核对真实源码 `pal/PalDefs.h` 的 `pal_stream_type_t` 枚举（值 1~27，最大为 `PAL_STREAM_PLAYBACK_BUS = 27`），本附录早期版本引用的以下流类型**在真实枚举中不存在，均为虚构**，已从各映射表中删除或标注：
> - `PAL_STREAM_SPATIAL_AUDIO` — 不存在（空间音频在 SA8295 上由 framework/effect 层处理，PAL 侧走普通 PCM 流）
> - `PAL_STREAM_PCM_HFP` — 不存在（HFP 免提相关的是 `PAL_STREAM_LOOPBACK_HFP_RX/TX`，非 `PCM_HFP`）
> - `PAL_STREAM_NAVI` — 不存在（导航音在 PAL 侧仍是普通 `PAL_STREAM_LOW_LATENCY/DEEP_BUFFER` 播放流，区分由 framework 的 usage/bus 决定）
> - `PAL_STREAM_GENERIC_CHIME` — 不存在（提示音同样走普通播放流）
>
> 真实完整枚举请参见 [15.4 流类型 pal_stream_type_t](15_15.4_流类型_pal_stream_type_t.md)。下文表格中涉及上述类型的行已改为对应真实流类型或标注为"framework 层概念（非 PAL 枚举）"。
...
`Session::makeSession()` 根据 `pal_stream_attributes->type` 用 `switch` 选择 Session 子类（源码 `session/src/Session.cpp:83`），实际只有 4 个分支：

| Session子类 | 对应的 pal_stream_type_t | 数据通路特征 |
|-------------|--------------------------|-------------|
| `SessionAlsaCompress` | `PAL_STREAM_COMPRESSED` | DSP Offload，压缩数据直传DSP解码 |
| `SessionAlsaVoice` | `PAL_STREAM_VOICE_CALL` | DSP Voice通路，VSID管理 |
| `SessionAgm` | `PAL_STREAM_NON_TUNNEL` | Non-Tunnel模式，经 AGM 直接图管理（绕过 DSP 渲染） |
| `SessionAlsaPcm` | **default**（其余所有类型：LOW_LATENCY / DEEP_BUFFER / ULTRA_LOW_LATENCY / VOIP / PROXY / HAPTICS / PLAYBACK_BUS / RAW / GENERIC / PCM_OFFLOAD 等） | DSP Tunnel 模式，经 AGM→GSL 图管理 |

> **重要更正**：早期文档曾以 `SessionGsl` 作为主 DSP 通路 Session，但源码 `Session::makeSession()` 中**不存在该分支**，`SessionGsl.cpp` 也已从代码库移除（仅遗留 `SessionGsl.h`）。普通 PCM 播放/录制走的是 **default 分支的 `SessionAlsaPcm`**；GSL 图管理由 `SessionAlsaPcm`/`SessionAgm` 经 AGM Service 间接完成，详见 [16.11 GSL接口与PAL Session的图管理](../16_Vendor_QNX_Architecture/16_16.11_SessionGsl与GSL接口.md)。
...
> **Session 选择逻辑（对照源码）**：`makeSession()` 是纯 `switch(type)` 静态分发，**没有** "GSL 是否可用" 的运行时判断——普通 PCM 类流（LOW_LATENCY/DEEP_BUFFER/ULL/VOIP/PROXY/HAPTICS/PLAYBACK_BUS 等）一律落入 default 分支创建 `SessionAlsaPcm`。所谓 "Tunnel/Non-Tunnel" 是 `SessionAlsaPcm` 内部依据设备与配置选择的图拓扑，而非切换到另一个 Session 类。SA8295 上 `SessionAlsaPcm` 经 AGM→gsl_fe→MM-HAB→gsl_vm_be 跨 VM 调用 DSP（GSL 图管理）。下表 Session 列的历史 `SessionGsl` 均应理解为 `SessionAlsaPcm`。
...
|------------|-----------|-------------|---------------|---------|
| `PAL_STREAM_LOW_LATENCY` | StreamPCM | SessionAlsaPcm / SessionAlsaPcm | Speaker / Headphone / BT A2DP | Tunnel / Non-Tunnel |
| `PAL_STREAM_DEEP_BUFFER` | StreamPCM | SessionAlsaPcm / SessionAlsaPcm | Speaker / Headphone / BT A2DP | Tunnel |
...
| `PAL_STREAM_ULTRA_LOW_LATENCY` | StreamPCM | SessionAlsaPcm | Speaker / Headphone | Tunnel (短延迟) |
| `PAL_STREAM_GENERIC` | StreamPCM | SessionAlsaPcm | Speaker / Headphone | Tunnel |
| `PAL_STREAM_RAW` | StreamPCM | SessionAlsaPcm | Speaker / Headphone | Tunnel (原始PCM) |
| `PAL_STREAM_HAPTICS` | StreamPCM | SessionAlsaPcm | HapticsDev | Tunnel |
| `PAL_STREAM_PLAYBACK_BUS` | StreamPCM | SessionAlsaPcm | A2BSpeaker / A2B2Speaker | Tunnel (AAOS TDM) |
| `PAL_STREAM_NON_TUNNEL` | StreamNonTunnel | SessionAlsaPcm | Speaker / Headphone | Non-Tunnel |
| `PAL_STREAM_PROXY` | StreamPCM | SessionAlsaPcm | RTProxy (Out) | Tunnel |
...
| `PAL_STREAM_VOIP` | StreamPCM | SessionAlsaPcm | Speaker+SpeakerMic / Headphone+HeadsetMic / BT SCO | Tunnel双向+AEC |
| `PAL_STREAM_VOIP_RX` | StreamPCM | SessionAlsaPcm | Speaker / Headphone | Tunnel (仅下行) |
| `PAL_STREAM_VOIP_TX` | StreamPCM | SessionAlsaPcm | SpeakerMic / HeadsetMic | Tunnel (仅上行) |
| `PAL_STREAM_VOICE_CALL_RECORD` | StreamInCall | SessionAlsaPcm | Proxy (In) | Tunnel (录音) |
| `PAL_STREAM_VOICE_CALL_TX` | StreamInCall | SessionAlsaPcm | Proxy (In) | Tunnel (上行录音) |
| `PAL_STREAM_VOICE_CALL_RX_TX` | StreamInCall | SessionAlsaPcm | Proxy (In) | Tunnel (双向录音) |
| `PAL_STREAM_VOICE_CALL_MUSIC` | StreamInCall | SessionAlsaPcm | Speaker / Headphone | Tunnel (通话背景音) |
...
| `PAL_STREAM_SENSOR_PCM_DATA` | StreamSensorPCMData | SessionAlsaPcm | SpeakerMic / HandsetVaMic | Tunnel (传感器) |

### 15.99.3.4 特殊流映射

| Stream类型 | Stream子类 | Session子类 | 典型Device | 音频路径 |
|------------|-----------|-------------|-----------|---------|
| `PAL_STREAM_LOOPBACK` | StreamPCM | SessionAlsaPcm | RTProxy (In+Out) | DSP Loopback |
| `PAL_STREAM_TRANSCODE` | StreamPCM | SessionAlsaPcm | Proxy | DSP转码 |
| `PAL_STREAM_ULTRASOUND` | StreamUltraSound | SessionAlsaPcm | UltrasoundDev + UltrasoundMic | DSP超声波 |
...
> **SA8295路径特化**：Tunnel模式下SessionAlsaPcm通过AGM→gsl_fe→MM-HAB→gsl_vm_be跨VM调用QNX侧GSL，最终由GSL管理DSP图。Non-Tunnel模式下SessionAlsaPcm直接操作ALSA设备节点，不经过跨VM调用。
...
|-----------|---------|-----------|---------|------|
| PAL_STREAM_PLAYBACK_BUS | SessionAlsaPcm | PAL_DEVICE_OUT_A2B_SPKR | TDM TX | 前排A2B区域 |
| PAL_STREAM_PLAYBACK_BUS | SessionAlsaPcm | PAL_DEVICE_OUT_A2B2_SPKR | TDM TX | 后排A2B区域 |
| PAL_STREAM_LOOPBACK | SessionAlsaPcm | RTProxy | TDM RX | 车内录音/回声参考 |
...
```
区域1(前排): PAL_STREAM_PLAYBACK_BUS → SessionAlsaPcm → A2BSpeaker → TDM1 TX
区域2(后排): PAL_STREAM_PLAYBACK_BUS → SessionAlsaPcm → A2B2Speaker → TDM2 TX
导航提示:    PAL_STREAM_LOW_LATENCY → SessionAlsaPcm → A2BSpeaker → TDM1 TX (混音)
```
...
|---------|--------|--------|--------|
| Tunnel播放 | SessionAlsaPcm → AGM → gsl_fe | → HAB → | gsl_vm_be → GSL → ADSP |
...
> **⚠️ 源码核实**：本表 `app_type`/PCM 前端设备为示意映射，真实取值以设备实际的 `resourcemanager_monaco.xml` 配置为准，不同 BSP 版本可能不同。表中已删除虚构流类型 `NAVI`/`GENERIC_CHIME` 行——导航音与提示音在 PAL 侧并无独立流类型，实际由 framework 按 usage/bus 归入 `LOW_LATENCY`/`DEEP_BUFFER` 等真实流类型播放。
