...
> [← 上一个](15_6.1_架构地位.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_8.1_HIDL_接口定义.md)

---

## 15.7 PayloadBuilder载荷构建器
...
graph TB
    ...
    GKV_Builder --> SessionGsl
    CKV_Builder --> SessionGsl
    TKV_Builder --> SessionGsl
    Module_Payload --> SessionGsl
    SessionGsl --> AGM
    AGM --> GSL
```
...
### 15.7.7.8 configureGraph()完整流程

`SessionGsl::configureGraph()`是PayloadBuilder产物的消费方，展示了完整的配置流程：
...
### 15.7.7.10 与GSL/AGM的交互数据流

PayloadBuilder的最终产物通过SessionGsl传递给AGM/GSL，完整数据流如下：

```mermaid
graph LR
    PB[PayloadBuilder] -->|GKV/CKV/TKV| SG[SessionGsl]
...
| 阶段 | 接口 | 说明 |
|------|------|------|
| PayloadBuilder → SessionGsl | GKV/CKV/TKV结构体 | 内存传递，零拷贝 |
| SessionGsl → AGM | `agm_graph_open()`/`agm_graph_set_config()` | AGM公共API |
...
### 15.7.7.11 PayloadBuilder在PAL生命周期中的位置
