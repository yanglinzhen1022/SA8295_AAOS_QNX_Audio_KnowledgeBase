...
> [← 上一个](15_4.1_流类型_pal_stream_type_t.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_6.1_架构地位.md)]
...
Stream::open()
  ...
  │       ├── SessionGsl:: agm_session_open + gsl_graph_open
...
StreamPCM::write(buffer)
  ...
  │       ├── SessionGsl: agm_write(session_id, buffer)
...
|---------|--------|------|------------|------|
| 低延迟 | `LOW_LATENCY` / `ULTRA_LOW_LATENCY` | OUTPUT | SessionGsl | 触控音效、按键音、FAST_TRACK |
| 深缓冲 | `DEEP_BUFFER` | OUTPUT | SessionGsl | 音乐播放、视频音轨 |
| 代理/原始 | `PROXY` / `RAW` / `GENERIC` | OUTPUT | SessionGsl | 代理转发、原始PCM、通用流 |
| 触觉/总线 | `HAPTICS` / `PLAYBACK_BUS` | OUTPUT | SessionGsl | 振动马达、AAOS TDM总线 |
| VoIP | `VOIP` / `VOIP_RX` / `VOIP_TX` | I/O | SessionGsl | VoIP双向/下行/上行，含AEC/NS |
| 空间音频 | `SPATIAL_AUDIO` | OUTPUT | SessionGsl | 空间音频渲染 |
...
- **SessionGsl**（GSL可用时）：通过 AGM 接口构建 DSP 音频图，数据走 Tunnel 路径。StreamPCM 的 `write()` 调用 `SessionGsl::write()` → `agm_write()` → ALSA FE pcm_write
...
缓冲区大小由 `mStreamAttr->in_media_config.rate/channels/bit_width` 计算得出，Session 层负责将数据写入 ALSA FE 节点。对于 SessionGsl，数据写入 ALSA FE 的 PCM 缓冲区后由 DMA 搬运到 DSP；对于 SessionAlsaPcm，数据直接写入 ALSA PCM 设备节点。
...
graph TB
    ...
    Gsl -->|DSP直接检测| AgmGsl[SessionGsl<br/>DSP内SVA引擎]
```
...
- **SoundTriggerEngineGsl**：SVA 算法在 DSP 侧执行，通过 SessionGsl 构建 DSP 检测图，DSP 直接处理 Mic 输入
...
| 流类型 | 方向 | Session | 说明 |
|--------|------|---------|------|
| `PAL_STREAM_VOICE_CALL_RECORD` | INPUT | SessionGsl | 通话中录音，从Voice Proxy读取双向通话数据 |
| `PAL_STREAM_VOICE_CALL_MUSIC` | OUTPUT | SessionGsl | 通话中背景音乐，混入下行通路 |
...
- ACD 引擎在 DSP 侧运行，通过 SessionGsl 构建检测图
...
| `StreamSensorPCMData` | `SENSOR_PCM_DATA` | SessionGsl | 传感器音频数据采集（AMC/加速度计） |
| `StreamUltraSound` | `ULTRASOUND` | SessionGsl | 超声波近距检测，发射+接收回波 |
...
|------------|--------------------------|---------|
| `StreamPCM` | LOW_LATENCY, DEEP_BUFFER, ULTRA_LOW_LATENCY, PROXY, HAPTICS, RAW, PLAYBACK_BUS, VOIP, VOIP_RX, VOIP_TX, VOICE_CALL, GENERIC, SPATIAL_AUDIO | PCM播放/录制/通话 |
| `StreamCompress` | COMPRESSED, PCM_OFFLOAD | Offload压缩播放 |
...
- 多个流类型映射到同一子类（如12+种PCM类型都创建StreamPCM），子类内部根据具体type差异化处理
- `PAL_STREAM_PCM_OFFLOAD` 创建 StreamCompress（非StreamPCM），因为Offload需要compress API
...
| **与CarAudio配合** | `PAL_STREAM_PLAYBACK_BUS` → SessionGsl → A2BSpeaker |
...
- 触觉波形PCM数据，`PAL_STREAM_HAPTICS` → StreamPCM → SessionGsl → HapticsDev
...
graph TB
    ...
    subgraph AudioSession[音频数据通路Session]
        SessionGsl[SessionGsl<br/>AGM/GSL音频图管理]
        SessionAlsaPcm[SessionAlsaPcm<br/>ALSA PCM设备操作]
...
- 音频数据通路 Session（SessionGsl / SessionAlsaPcm / SessionAlsaCompress / SessionAlsaVoice）面向音频数据传输
...
## 15.7.4 SessionGsl — GSL 音频图会话

### 15.7.4.1 类定义与核心成员

> 源码路径：`session/inc/SessionGsl.h`、`session/src/SessionGsl.cpp`

SessionGsl 是 AudioReach 架构下最核心的 Session 实现，管理 GSL 音频图的完整生命周期。绝大多数 PCM 类流都通过 SessionGsl 驱动：
...
SessionGsl 的核心方法通过 AGM API 间接调用 GSL，形成完整的调用链：

```mermaid
graph LR
    StreamOpen[Stream::open] --> SessOpen[SessionGsl::open]
    SessOpen --> PB1[PayloadBuilder::populateGkv]
    PB1 --> AGM1[agm_graph_open]
    AGM1 --> GSL1[gsl_graph_open]

    StreamStart[Stream::start] --> SessStart[SessionGsl::start]
    SessStart --> AGM2[agm_graph_start]
    AGM2 --> GSL2[gsl_graph_start]

    StreamWrite[Stream::write] --> SessWrite[SessionGsl::write]
    SessWrite --> AGM3[agm_session_write]
    AGM3 --> GSL3[gsl_graph_write]

    StreamStop[Stream::stop] --> SessStop[SessionGsl::stop]
    SessStop --> AGM4[agm_graph_stop]
    AGM4 --> GSL4[gsl_graph_stop]

    StreamClose[Stream::close] --> SessClose[SessionGsl::close]
...
SessionGsl::open() 是最复杂的方法，需要完成 KV 构建、图打开和配置下发：

```
SessionGsl::open(Stream *s)
...
在 SA8295 平台上，SessionGsl 的调用路径具有跨 VM 特征：
...
- 此路径对 SessionGsl 代码完全透明——AGM API 签名不变，跨 VM 逻辑封装在 libagm.so 内部
- 时延增加约 0.5-2ms（取决于 MM-HAB 通道负载），对低延迟流有影响

## 15.7.5 SessionAlsaPcm — ALSA PCM 设备会话

> 源码路径：`session/inc/SessionAlsaPcm.h`、`session/src/SessionAlsaPcm.cpp`

SessionAlsaPcm 通过 tinyalsa 直接操作 ALSA PCM 设备，用于不需要 DSP 图处理的简单 PCM 流场景：

```cpp
class SessionAlsaPcm : public Session {
private:
    std::vector<int> pcmDevIds;     // PCM设备ID列表
    struct pcm* pcm;                // tinyalsa PCM句柄
    struct mixer* mixer;            // ALSA mixer句柄
    struct pcm_config pcmConfig;    // PCM配置参数
    bool isMMap;                    // 是否使用MMAP模式
    int subGraphId;                 // 子图ID
};
```
...
| 成员 | 类型 | 说明 |
|------|------|------|
| pcmDevIds | vector\<int\> | PCM 设备节点 ID 列表，如 hw:0,1 对应 ID=0,1 |
| pcm | struct pcm* | tinyalsa PCM 操作句柄，由 pcm_open() 返回 |
| mixer | struct mixer* | ALSA mixer 句柄，用于控制音量/路由等 mixer 参数 |
| pcmConfig | struct pcm_config | PCM 配置：采样率/位深/通道数/period_size 等 |
| isMMap | bool | MMAP 模式标志，AAOS 低延迟播放可能使用 MMAP |

**核心方法调用链**：

```
SessionAlsaPcm::open(Stream *s)
  ├── 1. rm->getAudioRoute() → 获取ALSA音频路由
  ├── 2. rm->getpcmDeviceId() → 获取PCM设备ID列表 → pcmDevIds
  ├── 3. mixer_open(card) → 打开ALSA mixer
  ├── 4. pcm_open(card, device, flags, &pcmConfig) → 打开PCM设备
  └── 5. setupSessionDevice() → 配置设备参数

SessionAlsaPcm::write(Stream *s, tag, buf, size)
  ├── 1. 检查pcm句柄有效性
  ├── 2. pcm_write(pcm, buf->buffer, buf->size)
  └── 3. 更新 *size = 实际写入字节数

SessionAlsaPcm::read(Stream *s, tag, buf, size)
  ├── 1. 检查pcm句柄有效性
  ├── 2. pcm_read(pcm, buf->buffer, buf->size)
  └── 3. 更新 *size = 实际读取字节数
```

**SessionAlsaPcm 与 SessionGsl 的选择逻辑**：

在 AudioReach 架构下，Session::makeSession() 的选择取决于平台配置和流类型：
- 需要 DSP 处理（音效、回声消除等）→ SessionGsl
- 直通 PCM 不需 DSP → SessionAlsaPcm
- 实际选择由 ResourceManager 根据资源配置和 XML 决定
...
与 SessionGsl/SessionAlsaPcm 不同，SessionAlsaVoice 不通过 PCM/Compress 节点传输数据，而是通过 mixer 控制切换通话路由和参数。语音通话的数据通路由 Modem 和 DSP 直接处理，Android 侧只负责控制面的配置。
...
```
SessionGsl::setupSessionDevice(s, devId)
...
SessionGsl::connectSessionDevice(s, device)
...
|-------------------|-------------|---------|---------|
| PAL_STREAM_LOW_LATENCY | SessionGsl / SessionAlsaPcm | AGM/ALSA PCM | 触控音效、系统提示音 |
| PAL_STREAM_DEEP_BUFFER | SessionGsl / SessionAlsaPcm | AGM/ALSA PCM | 音乐播放 |
...
| PAL_STREAM_VOIP | SessionGsl | AGM | VoIP双向通话 |
...
| PAL_STREAM_ULTRA_LOW_LATENCY | SessionGsl | AGM | 超低延迟播放 |
| PAL_STREAM_PROXY | SessionGsl / SessionAlsaPcm | AGM/ALSA PCM | 代理PCM流 |
| PAL_STREAM_HAPTICS | SessionGsl | AGM | 触觉反馈振动 |
| PAL_STREAM_RAW | SessionAlsaPcm | ALSA PCM | 原始PCM流 |
| PAL_STREAM_PLAYBACK_BUS | SessionGsl | AGM | AAOS总线播放 |
...
1. **是否需要 DSP 处理**：需要 DSP 音效/EC/NS → SessionGsl
...
sequenceDiagram
    ...
    Factory->>Session: new SessionGsl/AlsaPcm/...
...
graph TB
    subgraph SessionHierarchy[Session类层次全貌]
        ...
        subgraph AudioPath[音频数据通路]
            SessionGsl[SessionGsl<br/>AGM/GSL音频图]
...
2. SessionGsl 是 AudioReach 架构的核心，通过 AGM API 管理 GSL 音频图，SA8295 上需跨 VM 到 QNX 侧执行
