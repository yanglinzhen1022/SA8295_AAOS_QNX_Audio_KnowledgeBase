...
> [← 上一个](15_15.3_API_分类总览.md) | [← 返回15章](README.md) | [返回导航](../README.md) | [下一个 →](15_15.5_核心类层次图-Stream-Device-Session.md)
...
`pal_stream_attributes` 是 `pal_stream_open()` 的核心参数，定义流的全部属性（字段以 `PalDefs.h` 真实定义为准）：

```c
struct pal_stream_attributes {
    pal_stream_type_t type;                    // 流类型，决定Stream子类和Session子类
    pal_stream_info_t info;                    // 流信息(联合体，按类型区分)
    pal_stream_flags_t flags;                  // 流标志位
    pal_stream_direction_t direction;          // 方向：PAL_AUDIO_INPUT/OUTPUT/INPUT_OUTPUT
    struct pal_media_config in_media_config;   // 输入媒体配置(结构体)
    struct pal_media_config out_media_config;  // 输出媒体配置(结构体)
    char* bus_addr;                            // BUS设备地址(AAOS Car Audio)
    uint32_t hal_flags;                        // HAL层传入的流标志
    audio_channel_mask_t ch_mask;              // audio_config传入的声道掩码
    audio_format_t format;                     // audio_config传入的音频格式
};
```

> ⚠️ **源码核实（勘误）**：旧版将 `in_media_config`/`out_media_config` 标为 `uint32_t` 采样率编码，且遗漏了 `info`/`bus_addr`/`hal_flags`/`ch_mask`/`format` 字段。真实 `PalDefs.h` 中二者均为 `struct pal_media_config`（含sample_rate/bit_width/aud_fmt_id/ch_info），完整字段如上。
...
| `info` | `pal_stream_info_t` | 流信息联合体，按流类型携带附加信息 |
| `flags` | `pal_stream_flags_t` | 流标志(TIMESTAMP/NON_BLOCKING/MMAP等) |
| `direction` | `pal_stream_direction_t` | 数据方向 OUTPUT/INPUT/INPUT_OUTPUT |
| `in_media_config` | `struct pal_media_config` | 输入侧媒体配置结构体 |
| `out_media_config` | `struct pal_media_config` | 输出侧媒体配置结构体 |
| `bus_addr` | `char*` | BUS设备地址，AAOS PLAYBACK_BUS流使用 |
| `hal_flags` | `uint32_t` | HAL层透传的流标志 |
| `ch_mask` | `audio_channel_mask_t` | 声道掩码 |
| `format` | `audio_format_t` | 音频格式 |
...
真实 `pal_stream_flags_t`（`PalDefs.h`，可 OR 组合）：

| 标志 | 值 | 说明 |
|------|-----|------|
| `PAL_STREAM_FLAG_TIMESTAMP` | 0x1 | 音频缓冲区附带时间戳 |
| `PAL_STREAM_FLAG_NON_BLOCKING` | 0x2 | 流IO操作非阻塞 |
| `PAL_STREAM_FLAG_MMAP` | 0x4 | MMAP模式(共享内存低延迟) |
| `PAL_STREAM_FLAG_MMAP_NO_IRQ` | 0x8 | MMAP无中断模式 |
| `PAL_STREAM_FLAG_EXTERN_MEM` | 0x10 | 共享内存缓冲由客户端分配 |
| `PAL_STREAM_FLAG_SRCM_INBAND` | 0x20 | MediaFormat变更事件随数据缓冲内联 |
| `PAL_STREAM_FLAG_EOF` | 0x40 | EOF标志 |

> ⚠️ **源码核实（勘误）**：旧版列出的 `PAL_STREAM_FLAG_NONTUNNEL`/`PAL_STREAM_FLAG_OFFLOAD`/`PAL_STREAM_FLAG_COMPRESS` 三个标志在真实 `PalDefs.h` 中**不存在**。真实标志如上表。非隧道(NON_TUNNEL)由流类型区分而非flags；Offload/Compress由流类型 `COMPRESSED`/`PCM_OFFLOAD` 决定。

> **方向与标志组合**：`direction` 决定数据流向，`flags` 决定IO行为(时间戳/非阻塞/MMAP等)。
...
struct pal_media_config {
    ...
    pal_audio_fmt_t aud_fmt_id;      // 音频格式ID
    struct pal_channel_info ch_info; // 声道信息(声道数+声道映射)
};
```

> ⚠️ **源码核实（勘误）**：真实 `pal_media_config` 无独立 `channels` 字段，声道数包含在 `ch_info`（`pal_channel_info`）中；且含 `aud_fmt_id`(音频格式ID)字段。
...
struct pal_buffer {
    uint8_t *buffer;                         // 数据指针
    size_t size;                             // 字节数
    size_t offset;                           // 有效数据在缓冲中的起始偏移
    struct timespec *ts;                     // 时间戳(指针)
    uint32_t flags;                          // 标志
    size_t metadata_size;                  // 元数据缓冲字节数
    uint8_t *metadata;                       // 元数据缓冲(可含多条)
    pal_extern_alloc_buff_info_t alloc_info; // 外部分配缓冲信息
};
```

> ⚠️ **源码核实（勘误）**：旧版将 `buffer` 标为 `void*`、`timestamp` 标为 `struct timespec` 值类型、并虚构了 `frames` 字段。真实 `pal_buffer`(`PalDefs.h`) 字段如上：`buffer` 为 `uint8_t*`、`ts` 为 `struct timespec*` 指针、含 `offset/flags/metadata/metadata_size/alloc_info`，**无 `frames` 字段**。
...
| `offset` | 有效数据起始偏移 |
| `ts` | 时间戳指针，用于AV同步 |
| `metadata`/`metadata_size` | 元数据缓冲及其大小 |
| `alloc_info` | 外部分配缓冲信息(EXTERN_MEM场景) |
...
struct pal_volume_data {
    ...
    struct pal_channel_vol_kv volume_pair[]; // 声道掩码-音量键值对数组
};

struct pal_channel_vol_kv {
    uint32_t channel_mask;                   // 声道掩码
    float vol;                               // 音量值 [0.0, 1.0]
};
```

> ⚠️ **源码核实（勘误）**：旧版虚构了 `pal_channel_vol{left, right}` 结构和 `channel_vol[]` 数组名。真实 `PalDefs.h` 中为 `no_of_volpairs` + `pal_channel_vol_kv volume_pair[]`，每个元素含 `channel_mask` 和音量值 `vol`（按声道掩码而非左右分设）。
...
> ⚠️ **源码核实（勘误）**：真实 `pal_stream_callback`(`PalDefs.h`) 中 `cookie` 为 `uint64_t`（非 `void*`），第4参数名为 `event_data_size`。其余参数（`stream_handle`/`event_id`/`event_data`）与源码一致。
...
| `cookie` | 用户上下文(uint64_t)，pal_stream_open时传入 |
